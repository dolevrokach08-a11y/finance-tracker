...
**הדוור דורש ש-CLI יהיה מחובר** — `claude auth status` אומר אם כן. אם לא:
`claude auth login`, או `claude setup-token` למשהו שרץ לבד. הדוור בודק את זה
לפני שהוא בונה worktree.
