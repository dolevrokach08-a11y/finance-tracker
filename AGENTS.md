...
- **ביקורת מספרית על Claude לא נופלת על GPT.** הוא לא בודק חישובים ולא אמור. ההגנה
  שם היא בדיקות אוטומטיות ותיעוד מפורש של כל מה שנגע במספר.
- **GPT בודק את המסך, לא את הדיף.** שם הערך שלו. סבב אחד כזה מצא ליקוי נגישות,
  התנהגות אקורדיון לא רצויה וגלישה אופקית — כולם בהסתכלות, לא בקריאת קוד.

### איך טענה הופכת למוסכמת

הנוהל הזה נכתב אחרי סבב שבו מחשבון עמלת הפירעון ירד מ-63,026 ל-15,390 מול 15,315
בדף הבנק. הוא מנוסח מהתקלות שקרו בו בפועל, לא מהתקלות שדמיינו.

1. **הסוכן הראשון מציג תוצאה, את ההנחות שמאחוריה, ותוצאה נצפית צפויה.**
   התחזית היא העיקר ולא קישוט: *"המסך יראה 15,389 מול 15,315 בדף"* — **נאמר לפני
   שמסתכלים**, אחרת זה דיווח ולא תחזית.
2. **הסוכן השני מחשב מחדש ממקורות הגלם ומנסה להפריך.** לא לקרוא את הדיף ולהנהן —
   לבנות את המספר מחדש מהמקור ולחפש דוגמה שסותרת. **ומה שנוסה ולא הפריך נכתב.**
   הסכמה בלי רשימת ניסיונות היא שני סוכנים מהנהנים; שלוש קונבנציות שנרשמו ככישלון
   הן מה שאיפשר לצד השני לקפוץ ישר לחלופה החסרה במקום לחזור על אותה דרך.
3. **נדרשת ראיה חיצונית.** צילום ממשק, הרצת workflow, השוואה למסמך בנק, או תרחיש
   ידוע מראש. שני סוכנים שמסכימים אינם ראיה.
4. **רק אז מותר לסמן "שנינו מסכימים".**
5. **וגם זה פירושו "מוכן לאישור", לא "מותר למזג".**
...
**הפילוח הוא הנקודה.** הסוכן השני תופס דברים שגויים **בתוצר**. דולב תפס דברים
שגויים **בשיפוט על מה להציג** — שורה שכתוב עליה "תקרה" והיא נמוכה מהאמת, מספר
סביר שנשען על קלט מנוחש. שני סוכנים יכולים להסכים לחלוטין על שניהם.

**לכן שלב 5 אינו פורמליות.** ביקורת אדברסרית היא שכבת הגנה מצוינת ואינה תחליף
לראיה חיצונית ולשער אישור אנושי.
...
- **תיעוד שמתאר תהליך ירוק. תיעוד שמצהיר על תוצאה — לא.** פתק בארכיון שכתוב בו
  "0.49% מהבנק" מטעה את הסבב הבא בדיוק כמו קוד שגוי.
- **CSS אינו בטוח מעצם היותו CSS.** שינוי ריפוד עבר בשלום בכרטיס שזה עתה נוספה לו
  שורת שדות שנייה — כי הסלקטור היה מדויק ומישהו בדק, לא כי CSS לא שובר מספרים.
...
**טענה מספרית עוברת דרך "איך טענה הופכת למוסכמת" למעלה** — תחזית לפני הבדיקה,
חישוב עצמאי מהמקור, ניסיונות הפרכה כתובים, ראיה חיצונית. "שנינו מסכימים" פירושו
מוכן לאישור.
...
לא כל בקשת עיצוב נראית אותו דבר, כי לא כל קוד תצוגה מרותך באותה מידה ללוגיקה.
**מי שמעביר חייב לומר לאיזה מסלול הדבר שייך** — זה לא אמור להיות ניחוש.
...
GPT מבקש בפתק, Claude מוסיף את הערך הנגזר, GPT מעצב מולו. הוא לא ממציא מספר,
Claude לא ממציא עיצוב.
...
סבב נחשב מוצלח רק אם הפתק **חזר שונה** מהבייטים שנמסרו ויש בו כותרת תגובה. קומיט
לבדו אינו תשובה — הדוור מקמט את הפתק שהוא עצמו מסר, וזה כבר נראה פעם כמו הצלחה.
...
ויוצאים בהצלחה. בלי הדגל הוא רץ עד `Ctrl-C`, כמו קודם.
...
**פתק שנערך תוך כדי סבב** אינו נשלח לפי הגרסה הישנה. הסריקה מצלמת את כל הפתקים
בהתחלה, וסבב יכול לקחת עשרים דקות, ולכן כל פתק נקרא שוב רגע לפני השיגור. אם השתנה —
הוא ממתין לסבב הבא. שינוי סיומות שורה אינו עריכה, כאן כמו בכל שאר המקומות.
