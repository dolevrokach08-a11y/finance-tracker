/* GENERATED from tax-optimizer.src.jsx — source sha256:1ac69e38f5500b3a
...
const mkBrackets = ceilings => ceilings.map((limit, i) => ({
  limit,
  rate: TAX_RATES[i]
})).concat([{
  limit: Infinity,
...
const TAX_PARAMS = {
  2022: {
    creditPoint: 2676,
    brackets: mkBrackets([77400, 110880, 178080, 247440, 514920, 660000])
  },
  // נק' זיכוי 223₪/חודש
  2023: {
    creditPoint: 2820,
    brackets: mkBrackets([81480, 116760, 187440, 260520, 542160, 698280])
  },
  // 235₪/חודש
  2024: {
    creditPoint: 2904,
    brackets: mkBrackets(FROZEN_BRACKETS)
  },
  // 242₪/חודש
  2025: {
    creditPoint: 2904,
...
function getTaxParams(year) {
  ...
  return {
    ...TAX_PARAMS[fallback],
    year: fallback,
    approx: Number.isFinite(y) && y > 0 && y !== fallback
  };
}
...
function calcGrant(monthly, nKids) {
  ...
  let base = 0;
  if (nKids >= 3) {
    if (monthly <= 4340) base = Math.min(420, (monthly - min) * 0.215);else if (monthly <= 5410) base = 420;else base = Math.max(0, 420 - (monthly - 5410) * 0.168);
  } else {
    if (monthly <= 4340) base = Math.min(290, (monthly - min) * 0.149);else if (monthly <= 5140) base = 290;else base = Math.max(0, 290 - (monthly - 5140) * 0.139);
  }
  const m = base * 1.5;
  return {
    grant: Math.round(m * 12),
    ok: m > 2
  };
}
...
function InfoTip({
  ...
  return /*#__PURE__*/React.createElement("span", {
    ...
    className: "tax-info-tip__bubble",
    style: {
      position: "absolute",
      bottom: "130%",
      right: "50%",
...
function Sl({
  ...
  return /*#__PURE__*/React.createElement("div", {
    className: `tax-slider${wide ? ' tax-slider--wide' : ''}`,
    style: {
      gridColumn: wide ? "1 / -1" : undefined
    }
  }, /*#__PURE__*/React.createElement("div", {
    className: "tax-slider__head",
    style: {
      display: "flex",
      alignItems: "center",
  ...
  }, label, tip && /*#__PURE__*/React.createElement(InfoTip, {
    text: tip
  })), /*#__PURE__*/React.createElement("span", {
    className: "tax-slider__value",
    style: {
      fontSize: 13,
      fontWeight: 700,
      color: "hsl(210, 20%, 92%)"
    }
  }, value < 0 ? "\u2014" : value.toLocaleString("he-IL"), " ", value >= 0 ? suf : "")), /*#__PURE__*/React.createElement("input", {
    type: "range",
    min: min,
    max: max,
    step: step,
...
function Tog({
  ...
  return /*#__PURE__*/React.createElement("button", {
    className: `tax-toggle${active ? ' is-active' : ''}`,
    "aria-pressed": active,
    onClick: onClick,
    style: {
      padding: "6px 14px",
      ...
      color: active ? "hsl(142, 60%, 50%)" : "hsl(215, 12%, 52%)",
...
function R({
  ...
  return /*#__PURE__*/React.createElement("div", {
    className: `tax-data-row${bold ? ' tax-data-row--total' : ''}`,
    style: {
      display: "flex",
      justifyContent: "space-between",
      fontSize: bold ? 14 : 12,
    ...
    className: "tax-data-row__label"
  }, /*#__PURE__*/React.createElement("i", {
    "aria-hidden": "true"
  }), label), /*#__PURE__*/React.createElement("span", {
    className: "tax-data-row__amount",
    style: {
      color: color || "hsl(210, 20%, 92%)"
    }
  }, amount));
}
...
function parsePayslipPDF(text) {
  ...
  new RegExp(`${SHK}\\s*תשלומים${GAP}${AMT}`), new RegExp(`${SHK}\\s*ברוטו${GAP}${AMT}`), new RegExp(`שכר\\s*ברוטו${GAP}${AMT}`), new RegExp(`ברוטו${GAP}${AMT}`), new RegExp(`תשלומים${GAP}${AMT}`),
  // Amount → Label (RTL PDF text extraction order)
  new RegExp(`${AMT}\\s*${SHK}\\s*תשלומים`), new RegExp(`${AMT}\\s*${SHK}\\s*ברוטו`), new RegExp(`${AMT}\\s*שכר\\s*ברוטו`), new RegExp(`${AMT}\\s*ברוטו`)]);
  result.net = tryMatch([
  // Label → Amount
  new RegExp(`נטו\\s*לתשלום${GAP}${AMT}`), new RegExp(`${SHK}\\s*נטו${GAP}${AMT}`), new RegExp(`שכר\\s*נטו${GAP}${AMT}`), new RegExp(`${SHK}\\s*לתשלום${GAP}${AMT}`), new RegExp(`העברה\\s*לבנק${GAP}${AMT}`), new RegExp(`סכום\\s*לתשלום${GAP}${AMT}`), new RegExp(`נטו\\s*להעברה${GAP}${AMT}`), new RegExp(`לתשלום${GAP}${AMT}`), new RegExp(`נטו${GAP}${AMT}`),
  // Amount → Label
  new RegExp(`${AMT}\\s*נטו\\s*לתשלום`), new RegExp(`${AMT}\\s*${SHK}\\s*נטו`), new RegExp(`${AMT}\\s*שכר\\s*נטו`), new RegExp(`${AMT}\\s*${SHK}\\s*לתשלום`), new RegExp(`${AMT}\\s*העברה\\s*לבנק`), new RegExp(`${AMT}\\s*סכום\\s*לתשלום`), new RegExp(`${AMT}\\s*נטו\\s*להעברה`), new RegExp(`${AMT}\\s*נטו`)]);
  ...
  const dedDefs = [{
    key: 'incomeTax',
    label: 'מס הכנסה',
    patterns: [new RegExp(`מס\\s*הכנסה${G}${A}`), new RegExp(`${A}\\s*מס\\s*הכנסה`), new RegExp(`ניכוי\\s*מס${G}${A}`), new RegExp(`${A}\\s*ניכוי\\s*מס`)]
  }, {
    key: 'nationalInsurance',
    label: 'ביטוח לאומי',
    patterns: [
    // Full name with flexible gap (handles "ביטוח לאומי עובד", "דמי ביטוח לאומי", etc.)
    new RegExp(`ביטוח\\s*לאומי${G}${A}`), new RegExp(`${A}\\s*ביטוח\\s*לאומי`), new RegExp(`דמי\\s*ביטוח\\s*לאומי${G}${A}`),
    // Abbreviations: בט"ל, ב.לאומי, ב"ל
    new RegExp(`בט${QUO}ל${G}${A}`), new RegExp(`${A}\\s*בט${QUO}ל`), new RegExp(`ב\\.\\s*לאומי${G}${A}`), new RegExp(`ב${QUO}ל[:\\s]*${A}`)]
  }, {
    key: 'healthTax',
    label: 'מס בריאות',
    patterns: [new RegExp(`מס\\s*בריאות${G}${A}`), new RegExp(`${A}\\s*מס\\s*בריאות`), new RegExp(`דמי\\s*בריאות${G}${A}`), new RegExp(`בריאות${G}${A}`), new RegExp(`${A}\\s*בריאות`)]
  }, {
    key: 'pension',
    label: 'פנסיה',
    patterns: [new RegExp(`פנסי[הת]${G}${A}`), new RegExp(`${A}\\s*פנסי[הת]`), new RegExp(`הפרשה\\s*לפנסיה${G}${A}`), /מנורה[^\\n]{0,15}?₪?\s*([0-9,]+\.?\d*)/, /מבטחים[^\\n]{0,15}?₪?\s*([0-9,]+\.?\d*)/, /גמל[^\\n]{0,15}?₪?\s*([0-9,]+\.?\d*)/, /הראל[^\\n]{0,15}?₪?\s*([0-9,]+\.?\d*)/, /מגדל[^\\n]{0,15}?₪?\s*([0-9,]+\.?\d*)/, /כלל\s*ביטוח[^\\n]{0,15}?₪?\s*([0-9,]+\.?\d*)/, /אלטשולר[^\\n]{0,15}?₪?\s*([0-9,]+\.?\d*)/]
  }, {
    key: 'hishtalmut',
    label: 'קרן השתלמות',
    patterns: [new RegExp(`קר[נן]\\s*השתלמות${G}${A}`), new RegExp(`${A}\\s*קר[נן]\\s*השתלמות`), new RegExp(`קרה${QUO}ש${G}${A}`), new RegExp(`${A}\\s*קרה${QUO}ש`), new RegExp(`השתלמות${G}${A}`)]
  }];
  for (const d of dedDefs) {
    const v = tryMatchDed(d.patterns);
    if (v) result.deductions[d.key] = {
      label: d.label,
      amount: v
    };
  }
...
function parseForm106(text) {
  ...
  result.annualGross = tryMatch106([new RegExp(`${SHK}\\s*הכנס[תה]\\s*עבודה${GAP106}${AMT106}`), new RegExp(`${SKH}\\s*הכנס[תה]\\s*עבודה${GAP106}${AMT106}`), new RegExp(`הכנס[תה]\\s*עבודה${GAP106}${AMT106}`), new RegExp(`${SHK}\\s*שכר\\s*ברוטו${GAP106}${AMT106}`), new RegExp(`${SHK}\\s*תשלומים${GAP106}${AMT106}`), new RegExp(`${SKH}\\s*תשלומים${GAP106}${AMT106}`), new RegExp(`הכנסה\\s*(?:חייבת|ברוטו|ממשכורת)${GAP106}${AMT106}`), new RegExp(`${SHK}\\s*ברוטו${GAP106}${AMT106}`), new RegExp(`שכר\\s*ברוטו\\s*שנתי${GAP106}${AMT106}`), new RegExp(`${SHK}\\s*שכר\\s*עבודה${GAP106}${AMT106}`), new RegExp(`${SKH}\\s*שכר\\s*עבודה${GAP106}${AMT106}`), new RegExp(`שכר\\s*עבודה${GAP106}${AMT106}`), new RegExp(`${SKH}\\s*ברוטו${GAP106}${AMT106}`),
  // Tax Authority portal format: "משכורת ותשלומים 1,874", "סה''כ (158/172) 1,874"
  new RegExp(`משכורת\\s*ותשלומים${GAP106}${AMT106}`), new RegExp(`${SHK}\\s*\\(158\\/172\\)${GAP106}${AMT106}`), new RegExp(`${SHK}\\s*\\(158${GAP106}${AMT106}`),
  // RTL order
  new RegExp(`${AMT106}\\s*${SHK}\\s*הכנס[תה]\\s*עבודה`), new RegExp(`${AMT106}\\s*${SKH}\\s*הכנס[תה]\\s*עבודה`), new RegExp(`${AMT106}\\s*הכנס[תה]\\s*עבודה`), new RegExp(`${AMT106}\\s*${SHK}\\s*שכר\\s*ברוטו`), new RegExp(`${AMT106}\\s*${SHK}\\s*תשלומים`), new RegExp(`${AMT106}\\s*הכנסה\\s*(?:חייבת|ברוטו|ממשכורת)`), new RegExp(`${AMT106}\\s*שכר\\s*עבודה`), new RegExp(`${AMT106}\\s*משכורת\\s*ותשלומים`)]);

  // Net patterns for 106
  result.annualNet = tryMatch106([new RegExp(`${SHK}\\s*נטו${GAP106}${AMT106}`), new RegExp(`${SKH}\\s*נטו${GAP106}${AMT106}`), new RegExp(`נטו\\s*לתשלום${GAP106}${AMT106}`), new RegExp(`${SHK}\\s*לתשלום${GAP106}${AMT106}`), new RegExp(`${SKH}\\s*לתשלום${GAP106}${AMT106}`), new RegExp(`שכר\\s*נטו\\s*שנתי${GAP106}${AMT106}`), new RegExp(`שכר\\s*נטו${GAP106}${AMT106}`),
  // RTL order
  new RegExp(`${AMT106}\\s*${SHK}\\s*נטו`), new RegExp(`${AMT106}\\s*${SKH}\\s*נטו`), new RegExp(`${AMT106}\\s*נטו\\s*לתשלום`), new RegExp(`${AMT106}\\s*שכר\\s*נטו`)]);
  ...
  const monthsMatch = text.match(/(?:מספר\s*)?חו?דש[יה]\s*(?:עבודה)?[:\s]*(\d{1,2})/) || text.match(/(\d{1,2})\s*חו?דש[יה]\s*עבודה/) || text.match(/חו?דש[יה]\s*עבודה[^]*?(?:V\s*)+(\d{1,2})\b/);
  if (monthsMatch) {
...
async function extractPDFText(arrayBuffer) {
  ...
  return {
    ltrText,
    rtlText,
    hasText: ltrText.replace(/\s/g, '').length > 20
  };
}
...
function extractNet(text, grossHint) {
  ...
  const labels = [`נטו\\s*לתשלום\\s*[:₪]*\\s*${N}`, `שכר\\s*נטו\\s*[:₪]*\\s*${N}`, `נטו\\s*להעברה\\s*[:₪]*\\s*${N}`, `סכום\\s*לתשלום\\s*[:₪]*\\s*${N}`, `העברה\\s*ל?בנק\\S*\\s*[:₪]*\\s*${N}`, `${N}\\s*נטו\\s*לתשלום`, `${N}\\s*שכר\\s*נטו`];
...
async function fetchCPIMap() {
  ...
  try {
    ...
    if (resp.ok) {
      ...
      if (map.size > 50) {
        try {
          localStorage.setItem(CPI_CACHE_KEY, JSON.stringify({
            entries: [...map],
            fetchedAt: Date.now(),
            source: 'cbs'
          }));
  ...
  try {
    const resp = await fetch(`${CF_WORKER_BASE}/cpi`, {
      signal: AbortSignal.timeout(8000)
    });
    if (resp.ok) {
      const data = await resp.json();
...
function getLatestCPI(cpiMap) {
  ...
  return {
    ...
    value: cpiMap.get(keys[keys.length - 1])
  };
}
...
function getRemainingLots(purchases, sales, symbol) {
  ...
  const lots = (purchases || []).filter(p => (p.symbol || '').toUpperCase() === sym).sort((a, b) => new Date(a.date) - new Date(b.date)).map(p => {
...
function calcSimulatedTax(holding, remainingLots, cpiMap, fxMap, currentRates) {
  ...
  const fxUsed = new Map(),
    fxMissing = new Set();
  const fxAt = cur => {
    const {
      rate,
      source
    } = currentFX(cur, currentRates, fxMap);
    if (rate === null) {
      fxMissing.add(cur);
      return null;
    }
  ...
  const lotsToUse = remainingLots.length > 0 ? remainingLots : [{
    date: holding.purchaseDate,
    price: fallbackCost,
    shares: Number(holding.shares || 0),
    currency,
    remaining: Number(holding.shares || 0)
  }];
  ...
  return {
    ...
    cpiAdjTax: Math.round(realTax),
    // kept name for backward compat with UI
    realTax: Math.round(realTax),
    adjustmentType,
    // 'cpi' | 'fx' | 'none' — for UI badge
...
function TaxBracketWaterfall({
  ...
  function computeBreakdown(annual) {
    ...
    for (let i = 0; i < brackets.length; i++) {
      ...
      result.push({
        from: prev,
        to: b.limit === Infinity ? prev + taxable : b.limit,
        rate: b.rate,
        taxable,
        tax,
        color: colors[i],
  ...
  function renderBar(breakdown, annual, totalTax, label, clr) {
    ...
    return /*#__PURE__*/React.createElement("div", {
      className: "tax-waterfall__person",
      style: {
        marginBottom: 16
      }
    }, /*#__PURE__*/React.createElement("div", {
      className: "tax-waterfall__person-head",
      style: {
        display: 'flex',
        justifyContent: 'space-between',
        alignItems: 'center',
      ...
      className: "tax-waterfall__person-label",
      style: {
        fontSize: 13,
        fontWeight: 600,
        color: clr
      }
    }, label, " \u2014 \u20AA", fmNum(annual), " \u05E9\u05E0\u05EA\u05D9"), /*#__PURE__*/React.createElement("span", {
      className: "tax-waterfall__tax",
      style: {
        fontSize: 13,
        fontWeight: 700,
        color: '#ef4444'
      }
    }, "\u05DE\u05E1: \u20AA", fmNum(totalTax), " (", annual > 0 ? (totalTax / annual * 100).toFixed(1) : 0, "%)")), /*#__PURE__*/React.createElement("div", {
      className: "tax-waterfall__track",
      style: {
        display: 'flex',
        height: 36,
        borderRadius: 8,
      ...
      return /*#__PURE__*/React.createElement("div", {
        className: "tax-waterfall__segment",
        key: b.label,
        style: {
          width: widthPct + '%',
          minWidth: widthPct > 3 ? '28px' : '0',
          background: b.color,
          display: 'flex',
          alignItems: 'center',
          justifyContent: 'center',
      ...
      className: "tax-waterfall__chips",
      style: {
        display: 'flex',
        gap: 6,
        marginTop: 6,
    ...
    }, breakdown.filter(b => b.taxable > 0).map(b => /*#__PURE__*/React.createElement("span", {
      className: "tax-waterfall__chip",
      key: b.label,
      style: {
        fontSize: 10,
        color: 'hsl(215,12%,52%)',
  ...
  return /*#__PURE__*/React.createElement("div", {
    className: "tax-panel tax-view tax-view--credits tax-waterfall",
    style: {
  ...
  }, "\uD83D\uDCCA \u05D5\u05D9\u05D6\u05D5\u05D0\u05DC\u05D9\u05D6\u05E6\u05D9\u05D9\u05EA \u05DE\u05D3\u05E8\u05D2\u05D5\u05EA \u05DE\u05E1"), /*#__PURE__*/React.createElement("div", {
    className: "tax-waterfall__legend",
    style: {
      display: 'flex',
      gap: 8,
      flexWrap: 'wrap',
    ...
    className: "tax-waterfall__swatch",
    style: {
      width: 12,
      height: 12,
      borderRadius: 3,
  ...
  }), labels[i]))), renderBar(fBreak, fatherAnnual, fTotalTax, '👨 האב', '#7ab8e0'), renderBar(mBreak, motherAnnual, mTotalTax, '👩 האם', '#d898c8'), /*#__PURE__*/React.createElement("div", {
    className: "tax-waterfall__total",
    style: {
      marginTop: 12,
      padding: '12px 16px',
  ...
  }, "\u05E1\u05D4\u05F4\u05DB \u05DE\u05E1 \u05D4\u05DB\u05E0\u05E1\u05D4 \u05DE\u05E9\u05E7 \u05D4\u05D1\u05D9\u05EA (\u05DC\u05E4\u05E0\u05D9 \u05D6\u05D9\u05DB\u05D5\u05D9\u05D9\u05DD)"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 26,
      fontWeight: 800,
      color: '#ef4444'
    }
  }, "\u20AA", fmNum(fTotalTax + mTotalTax)), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 11,
      color: 'hsl(215, 12%, 52%)'
    }
  }, "\u05E9\u05D9\u05E2\u05D5\u05E8 \u05DE\u05E1 \u05DE\u05DE\u05D5\u05E6\u05E2: ", fatherAnnual + motherAnnual > 0 ? ((fTotalTax + mTotalTax) / (fatherAnnual + motherAnnual) * 100).toFixed(1) : 0, "%")));
}
function App() {
  const taxData = window.__taxData || {
    loaded: false,
    years: {},
    availableYears: []
  };
  ...
  const runCPISim = useCallback(async () => {
    ...
    try {
      ...
      const priced = holdings.map(h => {
        ...
        const totalValueILS = own.rate === null ? 0 : Number(h.currentPrice || 0) * Number(h.shares || 0) * own.rate;
        ...
        return {
          ...
          totalValueILS: Math.round(totalValueILS),
          lotsFromHistory,
          fxMissing: missing,
          fxSources: sources
        };
      ...
      const fxStale = [...new Map(priced.flatMap(r => r.fxSources).filter(([, src]) => src !== 'live')).entries()].map(([currency, asOf]) => ({
        currency,
        asOf
      }));
      const rows = priced.filter(r => r.fxMissing.length === 0).filter(r => r.flatTax > 0 || r.cpiAdjTax > 0);
      const latest = getLatestCPI(cpiMap);
      // בדוק מקור הנתונים מה-cache
      let dataSource = 'static';
      try {
        dataSource = JSON.parse(localStorage.getItem(CPI_CACHE_KEY) || '{}').source || 'static';
      } catch (e) {}
      // FX latest month
    ...
    try {
      ...
      if (!netReliable) {
        try {
          ...
          if (!parsed.month && ocrParsed.month) {
            parsed.month = ocrParsed.month;
            parsed.year = ocrParsed.year;
          }
  ...
  const applyEarnerAverage = useCallback((earner, year) => {
    const slips = (window.__payslipData?.payslips || []).filter(p => p.earner === earner && p.month && p.month.startsWith(year + '-') && p.gross);
    ...
    if (earner === 'father') setFM(rounded);else if (earner === 'mother') setMM(rounded);
  }, []);
  const confirmForm106Upload = useCallback(async () => {
    ...
    if (!earner) {
      setPayslipStatus('יש לבחור בן זוג');
    ...
    if (!monthlyGross && !monthlyNet) {
      setPayslipStatus('לא זוהו נתוני הכנסה בטופס');
      setTimeout(() => setPayslipStatus(''), 3000);
      return;
    }
  ...
  const confirmPayslipUpload = useCallback(async () => {
    ...
    if (!pendingPayslip.gross && !pendingPayslip.net) {
      setPayslipStatus('יש להזין ברוטו או נטו');
      return;
    }
    const monthKey = pendingPayslip.month && pendingPayslip.year ? `${pendingPayslip.year}-${pendingPayslip.month}` : null;
    if (!monthKey) {
      setPayslipStatus('יש לבחור חודש');
      setTimeout(() => setPayslipStatus(''), 3000);
      return;
    }
  ...
  const payslipAverages = useMemo(() => {
    ...
    const calcAvg = earner => {
      ...
      return {
        avg,
        count: slips.length,
        months: monthTotals.length
      };
    };
    const f = calcAvg('father'),
      m = calcAvg('mother');
    return {
      fatherAvg: f.avg,
      motherAvg: m.avg,
      fatherCount: f.count,
      motherCount: m.count,
      fatherMonths: f.months,
      motherMonths: m.months
    };
  ...
  const updateChildAge = (id, age) => setChildren(prev => prev.map(c => c.id === id ? {
    ...c,
    age
  } : c));
  ...
  const r = useMemo(() => {
    ...
    const nK = ages.length,
      nT = ages.filter(a => a >= 0 && a < 3).length;
    ...
    const f = calc(fM, false),
      m = calc(mM, true);
    const fShare = jnt ? cg / 2 : cg,
      mShare = jnt ? cg / 2 : 0;
    const fCGTax = Math.round(fShare * CG_TAX_RATE),
      mCGTax = Math.round(mShare * CG_TAX_RATE);
    const fSaved = Math.min(fCGTax, f.unused),
      mSaved = Math.min(mCGTax, m.unused);
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: "tax-hero__signal",
    "aria-hidden": "true"
  }, /*#__PURE__*/React.createElement("i", null), /*#__PURE__*/React.createElement("i", null), /*#__PURE__*/React.createElement("i", null), /*#__PURE__*/React.createElement("b", null, "25%")), /*#__PURE__*/React.createElement("div", {
    className: "tax-hero__eyebrow",
    style: {
      fontSize: 12,
      color: "hsl(142, 60%, 50%)",
      letterSpacing: 3,
  ...
  }, "TAX OPTIMIZATION / LIVE MODEL"), /*#__PURE__*/React.createElement("h1", {
    style: {
      fontSize: 22,
      fontWeight: 800,
      margin: "0 0 6px",
  ...
  }, "\u05DE\u05E2\u05D1\u05D3\u05EA \u05D4\u05DE\u05E1 \u05E9\u05DC \u05DE\u05E9\u05E7 \u05D4\u05D1\u05D9\u05EA"), /*#__PURE__*/React.createElement("p", {
    style: {
      fontSize: 12,
      color: "hsl(215, 12%, 52%)",
      margin: 0
    }
  }, "\u05E0\u05E7\u05D5\u05D3\u05D5\u05EA \u05D6\u05D9\u05DB\u05D5\u05D9, \u05DE\u05E2\u05E0\u05E7\u05D9 \u05E2\u05D1\u05D5\u05D3\u05D4 \u05D5\u05E8\u05D5\u05D5\u05D7\u05D9 \u05D4\u05D5\u05DF \u05DE\u05EA\u05D7\u05D1\u05E8\u05D9\u05DD \u05DC\u05EA\u05D5\u05DB\u05E0\u05D9\u05EA \u05E4\u05E2\u05D5\u05DC\u05D4 \u05D0\u05D7\u05EA \u2014 \u05DC\u05E9\u05E0\u05D9 \u05D1\u05E0\u05D9 \u05D4\u05D6\u05D5\u05D2."), /*#__PURE__*/React.createElement("span", {
    className: "tax-hero__code"
  }, "MODULE 03 / TAX LAB")), /*#__PURE__*/React.createElement("nav", {
    className: "tax-workspace-tabs",
    "aria-label": "\u05D0\u05D6\u05D5\u05E8\u05D9 \u05DE\u05D7\u05E9\u05D1\u05D5\u05DF \u05D4\u05DE\u05E1"
  }, [['household', '01', 'משק הבית', 'הכנסות, תלושים וילדים'], ['credits', '02', 'מס וזיכויים', 'מדרגות, נקודות ומענקים'], ['capital', '03', 'רווחי הון', 'מימוש, קיזוז ומדד'], ['plan', '04', 'פירוט ותוכנית', 'טבלאות וצעדים לביצוע']].map(([id, n, title, sub]) => /*#__PURE__*/React.createElement("button", {
    key: id,
    className: workspace === id ? 'is-active' : '',
    "aria-current": workspace === id ? 'page' : undefined,
    onClick: () => setWorkspace(id)
  }, /*#__PURE__*/React.createElement("span", null, n), /*#__PURE__*/React.createElement("strong", null, title), /*#__PURE__*/React.createElement("small", null, sub)))), /*#__PURE__*/React.createElement("section", {
    className: "tax-kpi-strip",
    "aria-label": "\u05EA\u05DE\u05D5\u05E0\u05EA \u05DE\u05E6\u05D1 \u05E9\u05E0\u05EA\u05D9\u05EA"
  }, /*#__PURE__*/React.createElement("article", null, /*#__PURE__*/React.createElement("span", null, "\u05D4\u05DB\u05E0\u05E1\u05D4 \u05E9\u05E0\u05EA\u05D9\u05EA"), /*#__PURE__*/React.createElement("strong", null, fm((fM + mM) * 12), " \u20AA"), /*#__PURE__*/React.createElement("small", null, "\u05E9\u05E0\u05D9 \u05D1\u05E0\u05D9 \u05D4\u05D6\u05D5\u05D2")), /*#__PURE__*/React.createElement("article", null, /*#__PURE__*/React.createElement("span", null, "\u05D6\u05D9\u05DB\u05D5\u05D9 \u05D6\u05DE\u05D9\u05DF"), /*#__PURE__*/React.createElement("strong", null, fm(r.hUnused), " \u20AA"), /*#__PURE__*/React.createElement("small", null, "\u05DC\u05E7\u05D9\u05D6\u05D5\u05D6 \u05E8\u05D5\u05D5\u05D7\u05D9 \u05D4\u05D5\u05DF")), /*#__PURE__*/React.createElement("article", null, /*#__PURE__*/React.createElement("span", null, "\u05DE\u05E2\u05E0\u05E7\u05D9\u05DD \u05DE\u05E9\u05D5\u05E2\u05E8\u05D9\u05DD"), /*#__PURE__*/React.createElement("strong", null, fm(r.hGrant), " \u20AA"), /*#__PURE__*/React.createElement("small", null, "\u05DC\u05E9\u05E0\u05D4")), /*#__PURE__*/React.createElement("article", null, /*#__PURE__*/React.createElement("span", null, "\u05DE\u05D9\u05DE\u05D5\u05E9 \u05D0\u05D5\u05E4\u05D8\u05D9\u05DE\u05DC\u05D9"), /*#__PURE__*/React.createElement("strong", null, fm(r.hOpt), " \u20AA"), /*#__PURE__*/React.createElement("small", null, "\u05DC\u05E0\u05D9\u05E6\u05D5\u05DC \u05DE\u05DC\u05D5\u05D0 \u05D4\u05D6\u05D9\u05DB\u05D5\u05D9"))), TAX_PARAMS_STALE && /*#__PURE__*/React.createElement("div", {
    className: "tax-system-alert tax-system-alert--danger",
    style: {
      margin: "12px 14px",
      padding: "12px 16px",
  ...
  }, "\u26A0\uFE0F"), " \u05DE\u05D3\u05E8\u05D2\u05D5\u05EA \u05D4\u05DE\u05E1 \u05D1\u05D8\u05D1\u05DC\u05D4 \u05DE\u05E2\u05D5\u05D3\u05DB\u05E0\u05D5\u05EA \u05E2\u05D3 \u05E9\u05E0\u05EA ", LATEST_TAX_YEAR, ". \u05D7\u05D9\u05E9\u05D5\u05D1\u05D9\u05DD \u05DC\u05E9\u05E0\u05EA ", CURRENT_YEAR, " \u05DE\u05E9\u05EA\u05DE\u05E9\u05D9\u05DD \u05D1\u05E0\u05EA\u05D5\u05E0\u05D9 ", LATEST_TAX_YEAR, " \u2014 \u05DB\u05D3\u05D0\u05D9 \u05DC\u05E2\u05D3\u05DB\u05DF \u05D0\u05EA ", /*#__PURE__*/React.createElement("code", null, "TAX_PARAMS"), " \u05DE\u05D5\u05DC \u05E8\u05E9\u05D5\u05EA \u05D4\u05DE\u05E1\u05D9\u05DD."), taxData.loaded && taxData.availableYears.length > 0 && /*#__PURE__*/React.createElement("div", {
    className: "tax-system-alert tax-system-alert--data",
    style: {
      margin: "12px 14px",
      padding: "14px 16px",
  ...
  }, "\u05E0\u05EA\u05D5\u05E0\u05D9\u05DD \u05DE\u05D4\u05DE\u05E2\u05E8\u05DB\u05EA \u05D4\u05E4\u05D9\u05E0\u05E0\u05E1\u05D9\u05EA")), /*#__PURE__*/React.createElement("div", {
  ...
  }, "\u05E9\u05E0\u05D4:"), /*#__PURE__*/React.createElement("select", {
    value: selectedYear,
    onChange: e => {
  ...
  }, taxData.availableYears.map(y => /*#__PURE__*/React.createElement("option", {
    key: y,
    value: y,
    style: {
      background: "hsl(220, 18%, 10%)"
    }
  }, y, " ", taxData.years[y]?.isComplete ? "(12 חודשים)" : `(${taxData.years[y]?.monthCount} חודשים)`))), /*#__PURE__*/React.createElement("button", {
    onClick: () => applyYearData(selectedYear),
    style: {
      padding: "4px 10px",
      borderRadius: 6,
      cursor: "pointer",
  ...
  }, "\u05D4\u05D7\u05DC"))), yearInfo && /*#__PURE__*/React.createElement("div", {
    style: {
      marginTop: 10,
      display: "flex",
      flexWrap: "wrap",
  ...
  }, yearInfo.fatherMonthly > 0 && /*#__PURE__*/React.createElement("span", {
    style: {
      background: "rgba(122,184,224,0.1)",
      border: "1px solid rgba(122,184,224,0.25)",
      padding: "3px 10px",
      borderRadius: 6,
      color: "#7ab8e0"
    }
  }, "\uD83D\uDC68 \u05D0\u05D1: ", fm(yearInfo.fatherMonthly), " \u05E0\u05D8\u05D5", fRatio > 1.0 && /*#__PURE__*/React.createElement("span", {
    style: {
      color: "#5ab85a"
    }
  }, " \u2192 ", fm(Math.round(yearInfo.fatherMonthly * fRatio)), " \u05D1\u05E8\u05D5\u05D8\u05D5"), fRatio === 1.0 && " ₪/חודש"), yearInfo.motherMonthly > 0 && /*#__PURE__*/React.createElement("span", {
    style: {
      background: "rgba(216,152,200,0.1)",
      border: "1px solid rgba(216,152,200,0.25)",
      padding: "3px 10px",
      borderRadius: 6,
      color: "#d898c8"
    }
  }, "\uD83D\uDC69 \u05D0\u05DD: ", fm(yearInfo.motherMonthly), " \u05E0\u05D8\u05D5", mRatio > 1.0 && /*#__PURE__*/React.createElement("span", {
    style: {
      color: "#5ab85a"
    }
  }, " \u2192 ", fm(Math.round(yearInfo.motherMonthly * mRatio)), " \u05D1\u05E8\u05D5\u05D8\u05D5"), mRatio === 1.0 && " ₪/חודש"), yearInfo.untaggedMonthly > 0 && /*#__PURE__*/React.createElement("span", {
    style: {
  ...
  }, "\u26A0\uFE0F \u05DC\u05D0 \u05DE\u05E9\u05D5\u05D9\u05DA: ", fm(yearInfo.untaggedMonthly), " \u20AA/\u05D7\u05D5\u05D3\u05E9"), !yearInfo.isComplete && /*#__PURE__*/React.createElement("span", {
    style: {
      background: "rgba(224,112,112,0.1)",
  ...
  }, "\u05D7\u05DC\u05E7\u05D9 \u2014 ", yearInfo.monthCount, "/12 \u05D7\u05D5\u05D3\u05E9\u05D9\u05DD (\u05DE\u05DE\u05D5\u05E6\u05E2 \u05DE\u05D5\u05EA\u05D0\u05DD)")), yearInfo && yearInfo.untaggedMonthly > 0 && /*#__PURE__*/React.createElement("div", {
    style: {
      marginTop: 8,
      fontSize: 11,
      color: "hsl(142, 60%, 50%)",
  ...
  }, "\uD83D\uDCA1 \u05D9\u05E9 \u05D4\u05DB\u05E0\u05E1\u05D5\u05EA \u05E9\u05DC\u05D0 \u05E9\u05D5\u05D9\u05DB\u05D5 \u05DC\u05D0\u05D1/\u05D0\u05DD. ", /*#__PURE__*/React.createElement("a", {
    href: "finance.html",
    style: {
      color: "hsl(142, 60%, 50%)",
      textDecoration: "underline"
    }
  }, "\u05E9\u05D9\u05D9\u05DA \u05D0\u05D5\u05EA\u05DF \u05D1\u05DE\u05E2\u05E8\u05DB\u05EA \u05D4\u05E4\u05D9\u05E0\u05E0\u05E1\u05D9\u05EA"), " \u05DC\u05D7\u05D9\u05E9\u05D5\u05D1 \u05DE\u05D3\u05D5\u05D9\u05E7 \u05D9\u05D5\u05EA\u05E8."), /*#__PURE__*/React.createElement("div", {
    style: {
      marginTop: 8,
      fontSize: 10,
      color: "#6a6a8a",
  ...
  }, "\uD83D\uDCC5 \u05D4\u05D9\u05E1\u05D8 \u05D7\u05D5\u05D3\u05E9: \u05D4\u05DE\u05E9\u05DB\u05D5\u05E8\u05EA \u05D1\u05DE\u05E2\u05E7\u05D1 (15-15) \u05DE\u05D5\u05E1\u05D8\u05EA \u05D7\u05D5\u05D3\u05E9 \u05D0\u05D7\u05D5\u05E8\u05D4 \u05DC\u05E9\u05E0\u05EA \u05DE\u05E1 (\u05DC\u05DE\u05E9\u05DC: \"\u05D9\u05E0\u05D5\u05D0\u05E8\" \u05D1\u05DE\u05E2\u05E7\u05D1 = \u05D3\u05E6\u05DE\u05D1\u05E8 \u05DC\u05E6\u05D5\u05E8\u05DA \u05DE\u05E1)")), taxData.loaded && taxData.availableYears.length === 0 && /*#__PURE__*/React.createElement("div", {
    className: "tax-system-alert tax-system-alert--empty",
    style: {
      margin: "12px 14px",
      padding: "14px 16px",
      background: "rgba(76,175,80,0.08)",
  ...
  }, "\uD83D\uDCA1"), " \u05D0\u05D9\u05DF \u05E0\u05EA\u05D5\u05E0\u05D9 \u05D4\u05DB\u05E0\u05E1\u05D4 \u05D1\u05DE\u05E2\u05E8\u05DB\u05EA. ", /*#__PURE__*/React.createElement("a", {
    href: "finance.html",
    style: {
      color: "hsl(142, 60%, 50%)",
      textDecoration: "underline"
    }
  }, "\u05D4\u05D5\u05E1\u05E3 \u05D4\u05DB\u05E0\u05E1\u05D5\u05EA"), " \u05DB\u05D3\u05D9 \u05DC\u05DE\u05DC\u05D0 \u05D0\u05EA \u05D4\u05E1\u05DC\u05D9\u05D9\u05D3\u05E8\u05D9\u05DD \u05D0\u05D5\u05D8\u05D5\u05DE\u05D8\u05D9\u05EA."), /*#__PURE__*/React.createElement("div", {
    className: "tax-system-alert tax-system-alert--insight tax-view tax-view--capital",
    style: {
      margin: "16px 14px",
      padding: "14px 16px",
      background: "rgba(30,120,30,0.1)",
      border: "1px solid rgba(30,120,30,0.35)",
      borderRadius: 12
    }
  }, /*#__PURE__*/React.createElement("div", {
    style: {
  ...
  }, "\u05D0\u05D9\u05DF \u05E7\u05D5\u05E0\u05E4\u05DC\u05D9\u05E7\u05D8 \u2014 \u05E8\u05D5\u05D5\u05D7\u05D9 \u05D4\u05D5\u05DF \u05DC\u05D0 \u05E4\u05D5\u05D2\u05E2\u05D9\u05DD \u05D1\u05DE\u05E2\u05E0\u05E7")), /*#__PURE__*/React.createElement("p", {
    style: {
      fontSize: 12,
      lineHeight: 1.7,
      margin: 0,
  ...
  }, "\u05D4\u05D7\u05D5\u05E7 \u05DE\u05D2\u05D3\u05D9\u05E8 \"\u05D4\u05DB\u05E0\u05E1\u05D4 \u05D7\u05D9\u05D9\u05D1\u05EA\" ", /*#__PURE__*/React.createElement("strong", null, "\u05DC\u05DE\u05E2\u05D8 \u05E8\u05D5\u05D5\u05D7 \u05D4\u05D5\u05DF"), ". \u05DE\u05D9\u05DE\u05D5\u05E9 \u05E8\u05D5\u05D5\u05D7\u05D9 \u05D4\u05D5\u05DF \u05DC\u05D0 \u05DE\u05E9\u05E0\u05D4 \u05D0\u05EA \u05D4\u05D4\u05DB\u05E0\u05E1\u05D4 \u05DC\u05E6\u05D5\u05E8\u05DA \u05D6\u05DB\u05D0\u05D5\u05EA \u05DC\u05DE\u05E2\u05E0\u05E7 \u05E2\u05D1\u05D5\u05D3\u05D4.")), /*#__PURE__*/React.createElement("main", {
    className: "tax-workspace",
    style: {
      padding: "0 14px 32px",
      maxWidth: 720,
      margin: "0 auto"
    ...
    className: "tax-panel tax-view tax-view--household tax-household",
    style: {
      background: "rgba(255,255,255,0.035)",
      borderRadius: 14,
      border: "1px solid rgba(255,255,255,0.07)",
  ...
  }, "\u2699\uFE0F \u05E0\u05EA\u05D5\u05E0\u05D9 \u05DE\u05E9\u05E7 \u05D4\u05D1\u05D9\u05EA"), /*#__PURE__*/React.createElement("div", {
    className: "tax-person-grid",
    style: {
      display: "grid",
    ...
    step: 250,
    suf: "\u20AA",
    tip: "\u05D4\u05DB\u05E0\u05E1\u05EA \u05E2\u05D1\u05D5\u05D3\u05D4 \u05D1\u05E8\u05D5\u05D8\u05D5 \u05D1\u05DC\u05D1\u05D3"
  }), payslipAverages.fatherAvg && fM !== Math.round(payslipAverages.fatherAvg / 250) * 250 && /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      alignItems: "center",
      gap: 4,
  ...
  }, "\u05EA\u05DC\u05D5\u05E9\u05D9\u05DD: \u20AA", payslipAverages.fatherAvg.toLocaleString()), /*#__PURE__*/React.createElement("button", {
    onClick: () => setFM(Math.round(payslipAverages.fatherAvg / 250) * 250),
    style: {
      padding: "1px 8px",
      borderRadius: 4,
      cursor: "pointer",
  ...
  }, "\u05D4\u05D7\u05D6\u05E8 \u05DC\u05EA\u05DC\u05D5\u05E9\u05D9\u05DD"))), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement(Sl, {
    label: "\u05D4\u05DB\u05E0\u05E1\u05EA \u05D4\u05D0\u05DD (\u05D7\u05D5\u05D3\u05E9\u05D9 \u05D1\u05E8\u05D5\u05D8\u05D5)",
    value: mM,
    onChange: v => {
      setMM(v);
    },
    ...
    step: 250,
    suf: "\u20AA"
  }), payslipAverages.motherAvg && mM !== Math.round(payslipAverages.motherAvg / 250) * 250 && /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      alignItems: "center",
      gap: 4,
  ...
  }, "\u05EA\u05DC\u05D5\u05E9\u05D9\u05DD: \u20AA", payslipAverages.motherAvg.toLocaleString()), /*#__PURE__*/React.createElement("button", {
    onClick: () => setMM(Math.round(payslipAverages.motherAvg / 250) * 250),
    style: {
      padding: "1px 8px",
      borderRadius: 4,
      cursor: "pointer",
  ...
  }, "\u05D4\u05D7\u05D6\u05E8 \u05DC\u05EA\u05DC\u05D5\u05E9\u05D9\u05DD")))), /*#__PURE__*/React.createElement("div", {
    className: "tax-subpanel tax-payslip-uploader",
    style: {
      marginTop: 10,
      padding: "12px 14px",
    ...
    value: payslipEarner,
    onChange: e => setPayslipEarner(e.target.value),
    style: {
      background: "rgba(0,0,0,0.3)",
      border: "1px solid rgba(255,255,255,0.15)",
      color: "hsl(210, 20%, 92%)",
    ...
    value: ""
  }, "\u05D1\u05E2\u05DC/\u05EA \u05D4\u05DB\u05E0\u05E1\u05D4"), /*#__PURE__*/React.createElement("option", {
    value: "father"
  }, "\u05D0\u05D1"), /*#__PURE__*/React.createElement("option", {
    value: "mother"
  }, "\u05D0\u05DD")), /*#__PURE__*/React.createElement("label", {
    style: {
      padding: "5px 14px",
      borderRadius: 8,
      cursor: "pointer",
  ...
  }, "\u05D4\u05E2\u05DC\u05D4 \u05E7\u05D5\u05D1\u05E5", /*#__PURE__*/React.createElement("input", {
    ref: fileInputRef,
    type: "file",
  ...
  }, "\u05D4\u05D6\u05E0\u05D4 \u05D9\u05D3\u05E0\u05D9\u05EA"))), payslipStatus && /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 11,
      color: "#c0a0d8",
      marginBottom: 8
    }
  }, payslipStatus), pendingPayslip && /*#__PURE__*/React.createElement("div", {
    style: {
      background: pendingPayslip._isForm106 ? "rgba(76,175,80,0.08)" : "rgba(77,171,247,0.08)",
      border: `1px solid ${pendingPayslip._isForm106 ? "rgba(76,175,80,0.25)" : "rgba(77,171,247,0.2)"}`,
      borderRadius: 8,
      padding: "10px 12px",
      marginBottom: 8
    }
  }, pendingPayslip._isForm106 ? /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 12,
      fontWeight: 700,
      color: "hsl(142, 60%, 50%)",
  ...
  }, /*#__PURE__*/React.createElement("span", null, "\u05D8\u05D5\u05E4\u05E1 106 \u05D6\u05D5\u05D4\u05D4 \u2014 \u05E1\u05D9\u05DB\u05D5\u05DD \u05E9\u05E0\u05EA\u05D9"), /*#__PURE__*/React.createElement("input", {
    type: "number",
    min: "2020",
    max: "2030",
    value: pendingPayslip.year || '',
    onChange: e => setPendingPayslip(p => ({
      ...p,
  ...
  }, "\u05E0\u05EA\u05D5\u05E0\u05D9\u05DD \u05E9\u05E0\u05EA\u05D9\u05D9\u05DD"), pendingPayslip.annualGross && /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("span", {
    style: {
      color: "hsl(215, 12%, 52%)"
    }
  }, "\u05D1\u05E8\u05D5\u05D8\u05D5 \u05E9\u05E0\u05EA\u05D9: "), /*#__PURE__*/React.createElement("strong", {
    style: {
      color: "#5ab85a"
    }
  }, "\u20AA", pendingPayslip.annualGross.toLocaleString())), pendingPayslip.annualNet && /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("span", {
    style: {
      color: "hsl(215, 12%, 52%)"
    }
  }, "\u05E0\u05D8\u05D5 \u05E9\u05E0\u05EA\u05D9: "), /*#__PURE__*/React.createElement("strong", {
    style: {
      color: "#5ab85a"
    }
  }, "\u20AA", pendingPayslip.annualNet.toLocaleString())), /*#__PURE__*/React.createElement("div", {
    style: {
      color: "hsl(215, 12%, 52%)",
      marginTop: 2
    }
  }, "\u05D7\u05D5\u05D3\u05E9\u05D9 \u05E2\u05D1\u05D5\u05D3\u05D4: ", pendingPayslip.months)), /*#__PURE__*/React.createElement("div", {
    style: {
      background: "rgba(0,0,0,0.15)",
      borderRadius: 6,
      padding: "8px 10px"
  ...
  }, "\u05DE\u05DE\u05D5\u05E6\u05E2 \u05D7\u05D5\u05D3\u05E9\u05D9 (\u05D7\u05D9\u05E9\u05D5\u05D1)"), pendingPayslip.monthlyGross && /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("span", {
    style: {
      color: "hsl(215, 12%, 52%)"
    }
  }, "\u05D1\u05E8\u05D5\u05D8\u05D5: "), /*#__PURE__*/React.createElement("strong", {
    style: {
      color: "hsl(142, 60%, 50%)"
    }
  }, "\u20AA", pendingPayslip.monthlyGross.toLocaleString())), pendingPayslip.monthlyNet && /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("span", {
    style: {
      color: "hsl(215, 12%, 52%)"
    }
  }, "\u05E0\u05D8\u05D5: "), /*#__PURE__*/React.createElement("strong", {
    style: {
      color: "hsl(142, 60%, 50%)"
    }
  }, "\u20AA", pendingPayslip.monthlyNet.toLocaleString())))), !pendingPayslip.earner && !payslipEarner && /*#__PURE__*/React.createElement("div", {
    style: {
      color: "#e07070",
      fontSize: 10,
      marginTop: 6
    }
  }, "\u05D9\u05E9 \u05DC\u05D1\u05D7\u05D5\u05E8 \u05D1\u05DF \u05D6\u05D5\u05D2 (\u05D0\u05D1/\u05D0\u05DD) \u05DC\u05DE\u05E2\u05DC\u05D4 \u05DC\u05E4\u05E0\u05D9 \u05D4\u05D0\u05D9\u05E9\u05D5\u05E8"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 10,
      color: "hsl(215, 12%, 52%)",
      marginTop: 6
    }
  }, "\u05D9\u05D9\u05D5\u05D5\u05E6\u05E8\u05D5 ", pendingPayslip.months, " \u05E8\u05E9\u05D5\u05DE\u05D5\u05EA \u05D7\u05D5\u05D3\u05E9\u05D9\u05D5\u05EA \u05E2\u05D1\u05D5\u05E8 \u05E9\u05E0\u05EA ", pendingPayslip.year), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      gap: 6,
      marginTop: 8
    ...
    onClick: confirmForm106Upload,
    disabled: !pendingPayslip.earner && !payslipEarner,
    style: {
      padding: "5px 16px",
      borderRadius: 6,
      cursor: !pendingPayslip.earner && !payslipEarner ? "default" : "pointer",
      fontSize: 11,
      fontWeight: 600,
      border: "1px solid rgba(76,175,80,0.4)",
      background: "rgba(76,175,80,0.15)",
  ...
  }, "\u05D0\u05E9\u05E8 \u05D8\u05D5\u05E4\u05E1 106"), /*#__PURE__*/React.createElement("button", {
    onClick: () => setPendingPayslip(null),
    style: {
      padding: "5px 12px",
      borderRadius: 6,
      cursor: "pointer",
  ...
  }, "\u05D1\u05D8\u05DC"))) : pendingPayslip._manualEntry ? /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 11,
      fontWeight: 600,
      color: "#7ab8e0",
  ...
  }, pendingPayslip._scanned ? 'התלוש נראה כתמונה סרוקה — יש להזין ידנית:' : pendingPayslip._parseHint ? 'לא הצלחתי לזהות אוטומטית — הזן ידנית:' : 'הזנה ידנית:'), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "grid",
  ...
  }, "\u05D7\u05D5\u05D3\u05E9"), /*#__PURE__*/React.createElement("input", {
    type: "month",
    value: pendingPayslip.year && pendingPayslip.month ? `${pendingPayslip.year}-${pendingPayslip.month}` : '',
    onChange: e => {
  ...
  }, "\u05D1\u05E8\u05D5\u05D8\u05D5 (\u20AA)"), /*#__PURE__*/React.createElement("input", {
    type: "number",
    placeholder: "15,000",
    value: pendingPayslip.gross || '',
    onChange: e => setPendingPayslip(p => ({
      ...p,
      gross: e.target.value ? parseFloat(e.target.value) : null
    })),
    style: {
      background: "rgba(0,0,0,0.3)",
      border: "1px solid rgba(255,255,255,0.15)",
  ...
  }, "\u05E0\u05D8\u05D5 (\u20AA)"), /*#__PURE__*/React.createElement("input", {
    type: "number",
    placeholder: "11,000",
    value: pendingPayslip.net || '',
    onChange: e => setPendingPayslip(p => ({
      ...p,
      net: e.target.value ? parseFloat(e.target.value) : null
    })),
    style: {
      background: "rgba(0,0,0,0.3)",
      border: "1px solid rgba(255,255,255,0.15)",
  ...
  }, "\u05E0\u05EA\u05D5\u05E0\u05D9\u05DD \u05E9\u05D7\u05D5\u05DC\u05E6\u05D5:"), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "grid",
  ...
  }), !pendingPayslip.month && /*#__PURE__*/React.createElement("div", {
    style: {
      color: "#e07070",
      fontSize: 10,
      marginTop: 2
    }
  }, "\u05D1\u05D7\u05E8 \u05D7\u05D5\u05D3\u05E9")), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", {
    style: {
      color: "hsl(215, 12%, 52%)",
      display: "block",
      marginBottom: 2
  ...
  }, "\u05E0\u05D8\u05D5 (\u20AA)", pendingPayslip._netSource === 'ocr' && /*#__PURE__*/React.createElement("span", {
    title: "\u05E0\u05E7\u05E8\u05D0 \u05D1\u05E1\u05E8\u05D9\u05E7\u05EA OCR \u2014 \u05DE\u05D5\u05DE\u05DC\u05E5 \u05DC\u05D0\u05DE\u05EA",
    style: {
      color: "#c0a0d8",
      marginRight: 3
    }
    ...
    style: {
      background: "rgba(0,0,0,0.3)",
      border: `1px solid ${pendingPayslip._netSource === 'ocr' ? "rgba(192,160,216,0.5)" : "rgba(255,255,255,0.15)"}`,
      color: "#5ab85a",
      fontWeight: 700,
      borderRadius: 4,
      padding: "3px 6px",
  ...
  }, "\u05E0\u05D9\u05DB\u05D5\u05D9\u05D9\u05DD: "), /*#__PURE__*/React.createElement("strong", {
    style: {
      color: "#e07070"
    }
  }, pendingPayslip.gross && pendingPayslip.net ? `₪${(Math.round((pendingPayslip.gross - pendingPayslip.net) * 100) / 100).toLocaleString()}` : '?'))), pendingPayslip._netSource === 'ocr' && /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 10,
      color: "#c0a0d8",
      marginTop: 4
    }
  }, "\uD83D\uDD0D \u05D4\u05E0\u05D8\u05D5 \u05E0\u05E7\u05E8\u05D0 \u05D1\u05E1\u05E8\u05D9\u05E7\u05EA OCR (\u05DC\u05D0 \u05D4\u05D9\u05D4 \u05D6\u05DE\u05D9\u05DF \u05D1\u05E9\u05DB\u05D1\u05EA \u05D4\u05D8\u05E7\u05E1\u05D8) \u2014 \u05DB\u05D3\u05D0\u05D9 \u05DC\u05D0\u05DE\u05EA \u05DE\u05D5\u05DC \u05D4\u05EA\u05DC\u05D5\u05E9."), Object.keys(pendingPayslip.deductions || {}).length > 0 && /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 10,
      color: "hsl(215, 12%, 52%)",
      marginTop: 4
    }
  }, Object.values(pendingPayslip.deductions).map(d => `${d.label}: ₪${d.amount.toLocaleString()}`).join(' | '))), !pendingPayslip._isForm106 && /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      gap: 6,
      marginTop: 8
    ...
    onClick: confirmPayslipUpload,
    disabled: !pendingPayslip.gross && !pendingPayslip.net,
    style: {
      padding: "5px 16px",
      borderRadius: 6,
      cursor: !pendingPayslip.gross && !pendingPayslip.net ? "default" : "pointer",
      fontSize: 11,
      fontWeight: 600,
      border: "1px solid rgba(90,184,90,0.4)",
      background: "rgba(90,184,90,0.15)",
      color: "#5ab85a",
      opacity: !pendingPayslip.gross && !pendingPayslip.net ? 0.4 : 1
    }
  }, "\u05D0\u05E9\u05E8 \u05D5\u05E9\u05DE\u05D5\u05E8"), /*#__PURE__*/React.createElement("button", {
    onClick: () => setPendingPayslip(null),
    style: {
      padding: "5px 12px",
      borderRadius: 6,
  ...
  }, "\u05D1\u05D8\u05DC"))), (payslipAverages.fatherAvg || payslipAverages.motherAvg) && /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      flexWrap: "wrap",
      gap: 6,
  ...
  }, payslipAverages.fatherAvg && /*#__PURE__*/React.createElement("span", {
    style: {
      background: "rgba(122,184,224,0.1)",
  ...
  }, "\uD83D\uDC68 \u05D1\u05E8\u05D5\u05D8\u05D5 \u05DE\u05DE\u05D5\u05E6\u05E2: \u20AA", payslipAverages.fatherAvg.toLocaleString(), " (", payslipAverages.fatherMonths, " \u05D7\u05D5\u05D3\u05E9\u05D9\u05DD", payslipAverages.fatherCount > payslipAverages.fatherMonths ? `, ${payslipAverages.fatherCount} תלושים` : '', ")", /*#__PURE__*/React.createElement("button", {
    onClick: () => setFM(Math.round(payslipAverages.fatherAvg / 250) * 250),
    style: {
      marginRight: 4,
      padding: "1px 6px",
      ...
      fontWeight: 600,
      border: "1px solid rgba(90,184,90,0.4)",
      background: "rgba(90,184,90,0.1)",
      color: "#5ab85a"
    }
  }, "\u05D4\u05D7\u05DC")), payslipAverages.motherAvg && /*#__PURE__*/React.createElement("span", {
    style: {
      background: "rgba(216,152,200,0.1)",
      border: "1px solid rgba(216,152,200,0.2)",
      padding: "3px 8px",
      borderRadius: 6,
      color: "#d898c8"
    }
  }, "\uD83D\uDC69 \u05D1\u05E8\u05D5\u05D8\u05D5 \u05DE\u05DE\u05D5\u05E6\u05E2: \u20AA", payslipAverages.motherAvg.toLocaleString(), " (", payslipAverages.motherMonths, " \u05D7\u05D5\u05D3\u05E9\u05D9\u05DD", payslipAverages.motherCount > payslipAverages.motherMonths ? `, ${payslipAverages.motherCount} תלושים` : '', ")", /*#__PURE__*/React.createElement("button", {
    onClick: () => setMM(Math.round(payslipAverages.motherAvg / 250) * 250),
    style: {
      marginRight: 4,
      padding: "1px 6px",
  ...
  }, "\u05D4\u05D7\u05DC"))), payslips.length > 0 && /*#__PURE__*/React.createElement("div", {
    className: "tax-payslip-history"
  }, /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 11,
      fontWeight: 600,
  ...
  }, "\u05EA\u05DC\u05D5\u05E9\u05D9\u05DD (", payslips.length, "):"), /*#__PURE__*/React.createElement("div", {
    className: "tax-payslip-list",
    style: {
      display: "flex",
  ...
  }, [...payslips].sort((a, b) => (b.month || '').localeCompare(a.month || '') || (a.fileName || '').localeCompare(b.fileName || '')).map(p => /*#__PURE__*/React.createElement("div", {
    className: "tax-payslip-row",
    key: p.id,
    style: {
      display: "flex",
      alignItems: "center",
  ...
  }, /*#__PURE__*/React.createElement("span", null, p.month, " ", p.earner === 'father' ? '👨' : p.earner === 'mother' ? '👩' : ''), p.fileName && /*#__PURE__*/React.createElement("span", {
    title: p.fileName,
    style: {
      fontSize: 9,
      opacity: 0.6,
  ...
  }, p.fileName)), /*#__PURE__*/React.createElement("span", null, /*#__PURE__*/React.createElement("span", {
    style: {
      color: "hsl(215, 12%, 52%)"
    }
  }, "\u05D1\u05E8\u05D5\u05D8\u05D5: "), /*#__PURE__*/React.createElement("strong", {
  ...
  }, "\u20AA", p.gross?.toLocaleString() || '?'), /*#__PURE__*/React.createElement("span", {
    style: {
      color: "hsl(215, 12%, 52%)",
      marginRight: 8
    }
  }, " \u05E0\u05D8\u05D5: "), /*#__PURE__*/React.createElement("strong", null, "\u20AA", p.net?.toLocaleString() || '?')), /*#__PURE__*/React.createElement("button", {
  ...
  }, "\xD7")))))), payslips.length === 0 && /*#__PURE__*/React.createElement("div", {
    style: {
      marginTop: 10,
      padding: "10px 14px",
      background: "rgba(0,0,0,0.15)",
  ...
  }, "\u05D9\u05D7\u05E1 \u05D1\u05E8\u05D5\u05D8\u05D5/\u05E0\u05D8\u05D5"), /*#__PURE__*/React.createElement(InfoTip, {
    text: "הנתונים מ-ALLMON הם נטו (מה שנכנס לחשבון). יחס זה מכפיל את הנטו לברוטו משוער.\n\nלחלופין — העלה תלושי משכורת למעלה לחישוב מדויק.\n\n• 1.00 = כבר ברוטו\n• 1.07 = ללא מס (רק ביטוח לאומי+בריאות)\n• 1.20 = מדרגת 10%\n• 1.26 = מדרגת 14%"
  })), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "grid",
      gridTemplateColumns: "1fr 1fr",
  ...
  }, "\uD83D\uDC68 \u05D0\u05D1"), /*#__PURE__*/React.createElement("span", {
    style: {
      fontSize: 12,
      fontWeight: 700,
      color: "hsl(210, 20%, 92%)"
    }
  }, "\xD7", fRatio.toFixed(2))), /*#__PURE__*/React.createElement("input", {
    type: "range",
    min: 1.0,
    max: 1.5,
    step: 0.01,
    value: fRatio,
    onChange: e => setFRatio(+e.target.value)
  })), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      alignItems: "center",
  ...
  }, "\uD83D\uDC69 \u05D0\u05DD"), /*#__PURE__*/React.createElement("span", {
    style: {
      fontSize: 12,
      fontWeight: 700,
      color: "hsl(210, 20%, 92%)"
    }
  }, "\xD7", mRatio.toFixed(2))), /*#__PURE__*/React.createElement("input", {
    type: "range",
    min: 1.0,
    max: 1.5,
    step: 0.01,
  ...
  }, "\uD83D\uDC76 \u05D9\u05DC\u05D3\u05D9\u05DD (", children.length, ")"), /*#__PURE__*/React.createElement("button", {
    onClick: addChild,
    style: {
      padding: "5px 14px",
      borderRadius: 20,
  ...
  }, "+ \u05D4\u05D5\u05E1\u05E3 \u05D9\u05DC\u05D3")), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      flexDirection: "column",
      gap: 8
  ...
  }, "\u05D9\u05DC\u05D3 ", i + 1), /*#__PURE__*/React.createElement("input", {
    type: "range",
    min: 0,
    max: 18,
    step: 1,
  ...
  }, child.age === 0 ? "לידה" : `${child.age} ${child.age === 1 ? "שנה" : "שנים"}`), /*#__PURE__*/React.createElement("button", {
  ...
  }, "\xD7"))), children.length === 0 && /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 12,
      color: "#6a6a8a",
      textAlign: "center",
  ...
  }, "\u05D0\u05D9\u05DF \u05D9\u05DC\u05D3\u05D9\u05DD. \u05DC\u05D7\u05E5 \"\u05D4\u05D5\u05E1\u05E3 \u05D9\u05DC\u05D3\" \u05DB\u05D3\u05D9 \u05DC\u05D4\u05EA\u05D7\u05D9\u05DC."))), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      flexWrap: "wrap",
      gap: 10,
    ...
    step: 5000,
    suf: "\u20AA",
    tip: "\u05D1\u05D7\u05E9\u05D1\u05D5\u05DF \u05DE\u05E9\u05D5\u05EA\u05E3 \u05DE\u05EA\u05D7\u05DC\u05E7 50/50",
    wide: true
  }))), /*#__PURE__*/React.createElement(TaxBracketWaterfall, {
    fatherAnnual: fM * 12,
    motherAnnual: mM * 12,
    brackets: taxParams.brackets
  }), /*#__PURE__*/React.createElement("section", {
    className: "tax-panel tax-view tax-view--credits tax-credits",
    style: {
      background: "rgba(255,255,255,0.035)",
      borderRadius: 14,
      border: "1px solid rgba(255,255,255,0.07)",
  ...
  }, "\uD83D\uDCCA \u05E0\u05E7\u05D5\u05D3\u05D5\u05EA \u05D6\u05D9\u05DB\u05D5\u05D9"), /*#__PURE__*/React.createElement("span", {
    title: `מדרגות מס ושווי נק' זיכוי (${taxParams.creditPoint.toLocaleString()} ₪) של שנת ${taxParams.year}`,
    style: {
      fontSize: 11,
      fontWeight: 600,
      background: "rgba(122,184,224,0.12)",
      border: "1px solid rgba(122,184,224,0.3)",
      color: "#7ab8e0",
      padding: "3px 10px",
      borderRadius: 6
    }
  }, "\uD83D\uDDD3\uFE0F \u05D3\u05D9\u05E0\u05D9 \u05DE\u05E1 ", taxParams.year, taxParams.approx ? " (הקרובה ביותר)" : "")), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 11,
      color: "hsl(215, 12%, 52%)",
      marginBottom: 12,
  ...
  }, "\u05DC\u05E4\u05D9 \u05D4\u05E4\u05E7\u05D5\u05D3\u05D4 (\u05E1\u05E2' 11, 40) \u2014 \u05E0\u05E7\u05D5\u05D3\u05D5\u05EA \u05D9\u05DC\u05D3\u05D9\u05DD \u05D5\u05D6\u05D9\u05DB\u05D5\u05D9 \u05D9\u05D9\u05E9\u05D5\u05D1 \u05DE\u05D6\u05DB\u05D4 \u05DE\u05E7\u05D6\u05D6\u05D9\u05DD ", /*#__PURE__*/React.createElement("strong", null, "\u05E8\u05E7 \u05DE\u05E1 \u05E2\u05DC \u05D4\u05DB\u05E0\u05E1\u05D4 \u05DE\u05D9\u05D2\u05D9\u05E2\u05D4 \u05D0\u05D9\u05E9\u05D9\u05EA"), ". \u05E8\u05E7 \u05E0\u05E7' \u05EA\u05D5\u05E9\u05D1/\u05D0\u05D9\u05E9\u05D4 \u05DE\u05E7\u05D6\u05D6\u05D5\u05EA \u05D2\u05DD \u05E8\u05D5\u05D5\u05D7\u05D9 \u05D4\u05D5\u05DF."), /*#__PURE__*/React.createElement("div", {
    className: "tax-person-grid",
    style: {
      display: "grid",
      gridTemplateColumns: "1fr 1fr",
  ...
  }, "\u25C6 \u05DE\u05E7\u05D6\u05D6 \u05DB\u05DC \u05D4\u05DB\u05E0\u05E1\u05D4 (\u05DB\u05D5\u05DC\u05DC \u05E8\u05F4\u05D4)"), /*#__PURE__*/React.createElement(R, {
    label: `בסיס ${base}`,
    amount: `${fm(d.baseCr)} ₪`
  }), /*#__PURE__*/React.createElement("div", {
    style: {
  ...
  }, "\u25C6 \u05DE\u05E7\u05D6\u05D6 \u05E8\u05E7 \u05DE\u05E1 \u05E2\u05DC \u05D9\u05D2\u05D9\u05E2\u05D4 \u05D0\u05D9\u05E9\u05D9\u05EA"), /*#__PURE__*/React.createElement(R, {
    label: `ילדים (${d.childP.toFixed(1)} נק')`,
    amount: `${fm(d.childCr)} ₪`
  }), stl && /*#__PURE__*/React.createElement(R, {
    label: "\u05D6\u05D9\u05DB\u05D5\u05D9 \u05D9\u05D9\u05E9\u05D5\u05D1 \u05DE\u05D6\u05DB\u05D4",
    amount: `${fm(d.sCredit)} ₪`
  }), /*#__PURE__*/React.createElement("div", {
    style: {
      marginTop: 10,
    ...
    label: "\u05DE\u05E1 \u05E2\u05DC \u05D9\u05D2\u05D9\u05E2\u05D4 \u05D0\u05D9\u05E9\u05D9\u05EA",
    amount: `${fm(d.tax)} ₪`,
    color: "#e07070"
  }), d.workOnlyLost > 0 && /*#__PURE__*/React.createElement(R, {
    label: "\u05E0\u05E7' \u05D9\u05DC\u05D3\u05D9\u05DD/\u05D9\u05D9\u05E9\u05D5\u05D1 \u05E9\u05DE\u05EA\u05D0\u05D3\u05D5\u05EA",
    amount: `-${fm(d.workOnlyLost)} ₪`,
    color: "#a0808a"
  }), /*#__PURE__*/React.createElement(R, {
    label: "\u05D6\u05DE\u05D9\u05DF \u05DC\u05E7\u05D9\u05D6\u05D5\u05D6 \u05E8\u05F4\u05D4",
    amount: `${fm(d.unused)} ₪`,
    color: "hsl(142, 60%, 50%)",
    bold: true
  }))))), /*#__PURE__*/React.createElement("div", {
    className: "tax-result-callout tax-result-callout--credit",
    style: {
      marginTop: 12,
      padding: "12px 16px",
      background: "rgba(76,175,80,0.08)",
  ...
  }, "\u05E1\u05D4\u05F4\u05DB \u05D6\u05DE\u05D9\u05DF \u05DC\u05E7\u05D9\u05D6\u05D5\u05D6 \u05DE\u05E1 \u05E8\u05D5\u05D5\u05D7\u05D9 \u05D4\u05D5\u05DF"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 26,
      fontWeight: 800,
      color: "hsl(142, 60%, 50%)"
    }
  }, fm(r.hUnused), " \u20AA"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 11,
      color: "hsl(215, 12%, 52%)"
    }
  }, "\u05D4\u05D5\u05DC\u05DA \u05DC\u05D0\u05D9\u05D1\u05D5\u05D3 \u2014 \u05D0\u05DC\u05D0 \u05D0\u05DD \u05DE\u05DE\u05DE\u05E9\u05D9\u05DD \u05E8\u05D5\u05D5\u05D7\u05D9 \u05D4\u05D5\u05DF"))), /*#__PURE__*/React.createElement("section", {
    className: "tax-panel tax-view tax-view--credits tax-grants",
    style: {
      background: "rgba(255,255,255,0.035)",
      borderRadius: 14,
      border: "1px solid rgba(255,255,255,0.07)",
  ...
  }, "\uD83C\uDF81 \u05DE\u05E2\u05E0\u05E7 \u05E2\u05D1\u05D5\u05D3\u05D4"), /*#__PURE__*/React.createElement("div", {
    className: "tax-person-grid",
    style: {
      display: "grid",
      gridTemplateColumns: "1fr 1fr",
  ...
  }, icon, " ", lbl), d.wg.ok ? /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement(R, {
    label: "\u05DE\u05E2\u05E0\u05E7 \u05DB\u05DC\u05DC\u05D9",
    amount: `${fm(d.wg.grant)} ₪`
  }), /*#__PURE__*/React.createElement(R, {
    label: "\u05DE\u05E2\u05E0\u05E7 \u05E4\u05E2\u05D5\u05D8\u05D5\u05EA",
    amount: `${fm(d.tg)} ₪`
  }), /*#__PURE__*/React.createElement(R, {
    label: "\u05E1\u05D4\u05F4\u05DB",
    amount: `${fm(d.wg.grant + d.tg)} ₪`,
    color: "#5ab85a",
    bold: true
  })) : /*#__PURE__*/React.createElement("div", {
    style: {
  ...
  }, "\u26A0\uFE0F \u05D4\u05DB\u05E0\u05E1\u05D4 ", mo < 2390 ? "מתחת" : "מעל", " \u05DC\u05D8\u05D5\u05D5\u05D7", d.tg > 0 && /*#__PURE__*/React.createElement("div", {
    style: {
      color: "#a8cca8",
      marginTop: 4
    }
  }, "\u2713 \u05DE\u05E2\u05E0\u05E7 \u05E4\u05E2\u05D5\u05D8\u05D5\u05EA: ", fm(d.tg), " \u20AA"))))), /*#__PURE__*/React.createElement("div", {
    className: "tax-result-callout tax-result-callout--grant",
    style: {
      marginTop: 10,
      padding: "8px 14px",
      background: "rgba(30,120,30,0.08)",
      borderRadius: 8,
      fontSize: 12,
      color: "#a8cca8"
    }
  }, "\u05E1\u05D4\u05F4\u05DB \u05DE\u05E2\u05E0\u05E7\u05D9\u05DD: ", /*#__PURE__*/React.createElement("strong", null, fm(r.hGrant), " \u20AA/\u05E9\u05E0\u05D4"), " (\u05D4\u05E2\u05E8\u05DB\u05D4. ", /*#__PURE__*/React.createElement("a", {
    href: "https://www.misim.gov.il/gmmhszakaut/",
    target: "_blank"
  }, "\u05E1\u05D9\u05DE\u05D5\u05DC\u05D8\u05D5\u05E8"), " \u05DC\u05D7\u05D9\u05E9\u05D5\u05D1 \u05DE\u05D3\u05D5\u05D9\u05E7)")), /*#__PURE__*/React.createElement("section", {
    className: "tax-panel tax-view tax-view--capital tax-capital-gains",
    style: {
      background: "rgba(255,255,255,0.035)",
      borderRadius: 14,
  ...
  }, "\uD83C\uDFAF \u05DE\u05D9\u05DE\u05D5\u05E9 \u05E8\u05D5\u05D5\u05D7\u05D9 \u05D4\u05D5\u05DF"), /*#__PURE__*/React.createElement("div", {
    className: "tax-person-grid tax-capital-grid",
    style: {
      display: "grid",
      gridTemplateColumns: "1fr 1fr",
      gap: 12,
  ...
  }, x.lbl), /*#__PURE__*/React.createElement(R, {
    label: "\u05E8\u05D5\u05D5\u05D7 \u05D4\u05D5\u05DF",
    amount: `${fm(x.sh)} ₪`
  }), /*#__PURE__*/React.createElement(R, {
    label: "\u05DE\u05E1 25%",
    ...
    className: "tax-offset-meter",
    title: `נוצלו ${x.tx > 0 ? Math.min(100, x.sv / x.tx * 100).toFixed(0) : 0}% מהמס באמצעות זיכויים`
  }, /*#__PURE__*/React.createElement("span", null, /*#__PURE__*/React.createElement("i", {
    style: {
      width: `${x.tx > 0 ? Math.min(100, x.sv / x.tx * 100) : 0}%`
    }
  })), /*#__PURE__*/React.createElement("small", null, "\u05E7\u05D9\u05D6\u05D5\u05D6 \u05D0\u05E4\u05E7\u05D8\u05D9\u05D1\u05D9 ", /*#__PURE__*/React.createElement("b", null, x.tx > 0 ? Math.min(100, x.sv / x.tx * 100).toFixed(0) : 0, "%")))))), /*#__PURE__*/React.createElement("div", {
    className: "tax-optimal-callout",
    style: {
      padding: "14px 16px",
      borderRadius: 10,
    ...
    className: "tax-optimal-callout__orbit",
    "aria-hidden": "true"
  }, /*#__PURE__*/React.createElement("i", null), /*#__PURE__*/React.createElement("i", null)), /*#__PURE__*/React.createElement("div", {
    className: "tax-optimal-callout__copy"
  }, /*#__PURE__*/React.createElement("span", null, "\u05E1\u05DB\u05D5\u05DD \u05DE\u05D9\u05DE\u05D5\u05E9 \u05D0\u05D5\u05E4\u05D8\u05D9\u05DE\u05DC\u05D9"), /*#__PURE__*/React.createElement("strong", null, fm(r.hOpt), " \u20AA"), /*#__PURE__*/React.createElement("small", null, jnt ? `${fm(r.fOpt)} ₪ אב + ${fm(r.mOpt)} ₪ אם` : `מיוחס לאב`, " \xB7 \u05DE\u05E1 \u05D0\u05E4\u05E7\u05D8\u05D9\u05D1\u05D9 0%")), /*#__PURE__*/React.createElement("div", {
    className: "tax-optimal-callout__refund"
  }, /*#__PURE__*/React.createElement("span", null, "\u05D4\u05D7\u05D6\u05E8 \u05DE\u05E1 \u05E6\u05E4\u05D5\u05D9"), /*#__PURE__*/React.createElement("strong", null, fm(r.hUnused), " \u20AA")))), /*#__PURE__*/React.createElement("section", {
    className: "tax-panel tax-view tax-view--capital tax-bottom-line",
    style: {
  ...
  }, "\u2696\uFE0F \u05E9\u05D5\u05E8\u05D4 \u05EA\u05D7\u05EA\u05D5\u05E0\u05D4 \u2014 \u05E9\u05E0\u05EA\u05D9"), /*#__PURE__*/React.createElement("div", {
    className: "tax-scenario-grid",
    style: {
      display: "grid",
      gridTemplateColumns: "1fr 1fr 1fr",
      gap: 12
  ...
  }, "\u05DC\u05DC\u05D0 \u05DE\u05D9\u05DE\u05D5\u05E9"), /*#__PURE__*/React.createElement(R, {
    label: "\u05DE\u05E2\u05E0\u05E7\u05D9\u05DD",
    amount: `${fm(r.hGrant)} ₪`
  }), /*#__PURE__*/React.createElement(R, {
    label: "\u05D7\u05D9\u05E1\u05DB\u05D5\u05DF \u05DE\u05E1",
    ...
    className: "tax-scenario-card tax-scenario-card--actual",
    style: {
      padding: 14,
      borderRadius: 10,
  ...
  }, "\uD83D\uDCCC \u05DE\u05D9\u05DE\u05D5\u05E9 \u05D1\u05E4\u05D5\u05E2\u05DC (", fm(cg), " \u20AA)"), /*#__PURE__*/React.createElement(R, {
    label: "\u05DE\u05E2\u05E0\u05E7\u05D9\u05DD",
    amount: `${fm(r.hGrant)} ₪`
  }), /*#__PURE__*/React.createElement(R, {
    label: "\u05D7\u05D9\u05E1\u05DB\u05D5\u05DF \u05DE\u05E1",
    amount: `${fm(r.fSaved + r.mSaved)} ₪`,
    color: "#5ab85a"
  }), /*#__PURE__*/React.createElement(R, {
    label: "\u05E1\u05D4\u05F4\u05DB",
    amount: `${fm(r.hGrant + r.fSaved + r.mSaved)} ₪`,
    color: "#7ab8e0",
    bold: true
  })), /*#__PURE__*/React.createElement("div", {
    className: "tax-scenario-card tax-scenario-card--optimal",
    style: {
      padding: 14,
      borderRadius: 10,
      background: "rgba(30,120,30,0.06)",
      border: "1px solid rgba(30,120,30,0.25)"
    }
  }, /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 12,
  ...
  }, "\u2728 \u05DE\u05D9\u05DE\u05D5\u05E9 \u05D0\u05D5\u05E4\u05D8\u05D9\u05DE\u05DC\u05D9 (", fm(r.hOpt), " \u20AA)"), /*#__PURE__*/React.createElement(R, {
    label: "\u05DE\u05E2\u05E0\u05E7\u05D9\u05DD",
    amount: `${fm(r.hGrant)} ₪`
  }), /*#__PURE__*/React.createElement(R, {
    label: "\u05D7\u05D9\u05E1\u05DB\u05D5\u05DF \u05DE\u05E1",
    ...
    label: "\u05E1\u05D4\u05F4\u05DB",
    amount: `${fm(r.hGrant + r.hUnused)} ₪`,
    color: "#5ab85a",
    bold: true
  }))), /*#__PURE__*/React.createElement("div", {
    className: "tax-result-callout tax-result-callout--saving",
    style: {
      marginTop: 14,
      padding: "14px",
      borderRadius: 10,
  ...
  }, "\u05E8\u05D5\u05D5\u05D7 \u05E0\u05D5\u05E1\u05E3 \u05DE\u05DE\u05D9\u05DE\u05D5\u05E9 \u05D1\u05E4\u05D5\u05E2\u05DC"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 30,
      fontWeight: 800,
      color: "hsl(142, 60%, 50%)"
    }
  }, "+", fm(r.fSaved + r.mSaved), " \u20AA"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 12,
      color: "hsl(215, 12%, 52%)"
    }
  }, r.fSaved + r.mSaved < r.hUnused ? `ניתן לחסוך עוד ${fm(r.hUnused - r.fSaved - r.mSaved)} ₪ ע״י מימוש ${fm(r.hOpt - cg)} ₪ נוספים` : "ניצול מלוא הזיכויים! 🎯"))), /*#__PURE__*/React.createElement("section", {
    className: "tax-panel tax-view tax-view--capital tax-cpi-simulator",
    style: {
      background: "rgba(255,255,255,0.035)",
      borderRadius: 14,
  ...
  }, "\uD83D\uDCCA \u05DE\u05E1 \u05E8\u05D5\u05D5\u05D7\u05D9 \u05D4\u05D5\u05DF \u05E8\u05D9\u05D0\u05DC\u05D9 (\u05DE\u05D3\u05D3 / \u05E9\u05E2\u05E8 \u05D7\u05DC\u05D9\u05E4\u05D9\u05DF)"), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      gap: 8,
      alignItems: "center",
  ...
  }, cpiSim?.latestCPIDate && /*#__PURE__*/React.createElement("span", {
    style: {
      fontSize: 11,
      color: cpiSim.dataSource === 'cbs' ? "#5ab85a" : "hsl(215, 12%, 52%)",
      title: cpiSim.dataSource === 'cbs' ? 'נתוני הלמ"ס בזמן אמת' : 'הערכה על בסיס אינפלציה שנתית'
    }
  }, cpiSim.dataSource === 'cbs' ? '✓ מדד הלמ"ס' : '~מדד משוער', " \u05E2\u05D3 ", cpiSim.latestCPIDate), cpiSim?.latestFXDate && /*#__PURE__*/React.createElement("span", {
    style: {
      fontSize: 11,
      color: "hsl(215, 12%, 52%)",
      title: "שערי חליפין מ-Massive Market Data"
    }
  }, "\uD83D\uDCB1 FX \u05E2\u05D3 ", cpiSim.latestFXDate), /*#__PURE__*/React.createElement("button", {
    onClick: runCPISim,
    disabled: cpiLoading,
    style: {
      padding: "7px 14px",
      borderRadius: 8,
      cursor: cpiLoading ? "wait" : "pointer",
      fontSize: 12,
      fontWeight: 700,
  ...
  }, cpiLoading ? "⏳ טוען מדד..." : cpiSim ? "🔄 חשב מחדש" : "▶ חשב"))), /*#__PURE__*/React.createElement("p", {
    style: {
      fontSize: 12,
      color: "hsl(215, 12%, 52%)",
      margin: "0 0 12px",
  ...
  }, "\u05DE\u05D7\u05E9\u05D1 \u05D0\u05EA \u05DE\u05E1 \u05E8\u05D5\u05D5\u05D7\u05D9 \u05D4\u05D4\u05D5\u05DF \u05D4\u05E8\u05D9\u05D0\u05DC\u05D9 \u05D1\u05D4\u05E9\u05D5\u05D5\u05D0\u05D4 \u05DC\u05D7\u05D9\u05E9\u05D5\u05D1 \u05D4\u05E4\u05E9\u05D5\u05D8 \u05E9\u05DC 25% (\u05DB\u05D2\u05D5\u05DF \u05D0\u05E7\u05E1\u05DC\u05E0\u05E1). \u05D4\u05E1\u05D9\u05DE\u05D5\u05DC\u05E6\u05D9\u05D4 \u05DE\u05D1\u05D5\u05E1\u05E1\u05EA \u05E2\u05DC \u05DE\u05DB\u05D9\u05E8\u05D4 \u05D4\u05D9\u05E4\u05D5\u05EA\u05D8\u05D9\u05EA \u05D1\u05DE\u05D7\u05D9\u05E8 \u05D4\u05E0\u05D5\u05DB\u05D7\u05D9.", /*#__PURE__*/React.createElement("br", null), /*#__PURE__*/React.createElement("span", {
    style: {
      color: "#5ab85a"
    }
  }, "\u25CF \u05E0\u05D9\u05D9\u05E8\u05D5\u05EA \u05D1\u05E9\"\u05D7"), " \u2014 \u05E1\u05DE\u05DF \"\u05DE\u05D5\u05E6\u05DE\u05D3 \u05DC\u05DE\u05D3\u05D3\" ", /*#__PURE__*/React.createElement("a", {
    href: "portfolio.html",
    style: {
      color: "hsl(210,70%,65%)"
    }
  }, "\u05D1\u05D3\u05E3 \u05D4\u05E4\u05D5\u05E8\u05D8\u05E4\u05D5\u05DC\u05D9\u05D5"), " \u05DB\u05D3\u05D9 \u05DC\u05D4\u05D7\u05D9\u05DC \u05D4\u05E6\u05DE\u05D3\u05D4 \u05DC-CPI (\u05D0\u05D2\"\u05D7 \u05E6\u05DE\u05D5\u05D3\u05D5\u05EA, \u05D7\u05DC\u05E7 \u05DE\u05D4\u05E7\u05E8\u05E0\u05D5\u05EA).", /*#__PURE__*/React.createElement("br", null), /*#__PURE__*/React.createElement("span", {
    style: {
      color: "hsl(210,70%,65%)"
    }
  }, "\u25CF \u05E0\u05D9\u05D9\u05E8\u05D5\u05EA \u05D1\u05DE\u05D8\"\u05D7 (USD/EUR)"), " \u2014 \u05DE\u05E7\u05D1\u05DC\u05D9\u05DD \u05D0\u05D5\u05D8\u05D5\u05DE\u05D8\u05D9\u05EA \u05D4\u05E6\u05DE\u05D3\u05D4 \u05DC\u05E9\u05E2\u05E8 \u05D4\u05D7\u05DC\u05D9\u05E4\u05D9\u05DF \u05DC\u05E4\u05D9 \u05E1\u05E2\u05D9\u05E3 88 \u05DC\u05E4\u05E7\u05D5\u05D3\u05D4 (\u05DB\u05DC\u05DC \"\u05D4\u05E0\u05DE\u05D5\u05DA \u05DE\u05D4\u05E9\u05E0\u05D9\u05D9\u05DD\"). \u05D0\u05D9\u05DF \u05E6\u05D5\u05E8\u05DA \u05D1\u05E1\u05D9\u05DE\u05D5\u05DF."), cpiError && /*#__PURE__*/React.createElement("div", {
    style: {
      padding: "10px 14px",
      borderRadius: 8,
  ...
  }, "\u274C ", cpiError), cpiSim?.fxMissing?.length > 0 && /*#__PURE__*/React.createElement("div", {
    style: {
      padding: "10px 14px",
      borderRadius: 8,
      background: "rgba(200,80,80,0.1)",
  ...
  }, "\u26A0 \u05D0\u05D9\u05DF \u05E9\u05E2\u05E8 \u05D7\u05DC\u05D9\u05E4\u05D9\u05DF \u05DC-", cpiSim.fxMissing.join(', '), " \u2014 \u05E0\u05D9\u05D9\u05E8\u05D5\u05EA \u05D1\u05DE\u05D8\u05D1\u05E2\u05D5\u05EA \u05D4\u05D0\u05DC\u05D4 \u05D4\u05D5\u05E9\u05DE\u05D8\u05D5 \u05DE\u05D4\u05D7\u05D9\u05E9\u05D5\u05D1. \u05D4\u05E1\u05D9\u05DE\u05D5\u05DC\u05E6\u05D9\u05D4 \u05DB\u05D0\u05DF \u05D7\u05DC\u05E7\u05D9\u05EA."), cpiSim?.fxStale?.length > 0 && /*#__PURE__*/React.createElement("div", {
    style: {
      padding: "10px 14px",
      borderRadius: 8,
  ...
  }, "\uD83D\uDCB1 \u05D0\u05D9\u05DF \u05E9\u05E2\u05E8 \u05E2\u05D3\u05DB\u05E0\u05D9; \u05D7\u05D5\u05E9\u05D1 \u05DC\u05E4\u05D9 \u05D4\u05E9\u05E2\u05E8 \u05D4\u05D7\u05D5\u05D3\u05E9\u05D9 \u05D4\u05D0\u05D7\u05E8\u05D5\u05DF \u2014 ", cpiSim.fxStale.map(f => `${f.currency} מ-${f.asOf}`).join(', '), "."), cpiSim && cpiSim.rows.length === 0 && /*#__PURE__*/React.createElement("div", {
    style: {
      padding: "10px 14px",
      borderRadius: 8,
      background: "rgba(200,160,80,0.1)",
  ...
  }, "\u05D0\u05D9\u05DF \u05E8\u05D5\u05D5\u05D7\u05D9\u05DD \u05D7\u05D9\u05D5\u05D1\u05D9\u05D9\u05DD \u05DC\u05D4\u05E6\u05D2\u05D4 \u2014 \u05D9\u05D9\u05EA\u05DB\u05DF \u05E9\u05DE\u05D7\u05D9\u05E8\u05D9 \u05D4\u05E0\u05D9\u05D9\u05E8\u05D5\u05EA \u05DC\u05D0 \u05E2\u05D5\u05D3\u05DB\u05E0\u05D5, \u05D0\u05D5 \u05E9\u05D0\u05D9\u05DF \u05E8\u05D5\u05D5\u05D7\u05D9\u05DD \u05DC\u05D0 \u05DE\u05DE\u05D5\u05DE\u05E9\u05D9\u05DD."), cpiSim && cpiSim.rows.length > 0 && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
    className: "tax-cpi-summary",
    style: {
      display: "grid",
      gridTemplateColumns: "1fr 1fr 1fr",
      gap: 10,
    ...
    className: "tax-cpi-metric tax-cpi-metric--flat",
    style: {
      padding: 12,
      borderRadius: 10,
  ...
  }, "\u05DE\u05E1 \u05DC\u05E4\u05D9 \u05D0\u05E7\u05E1\u05DC\u05E0\u05E1 (25%)"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 20,
      fontWeight: 800,
      color: "#e07070"
    }
  }, fm(cpiSim.totalFlat), " \u20AA")), /*#__PURE__*/React.createElement("div", {
    className: "tax-cpi-metric tax-cpi-metric--real",
    style: {
      padding: 12,
      borderRadius: 10,
  ...
  }, "\u05DE\u05E1 \u05E8\u05D9\u05D0\u05DC\u05D9 (\u05DE\u05D3\u05D3 + FX)"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 20,
      fontWeight: 800,
      color: "hsl(210,70%,65%)"
    }
  }, fm(cpiSim.totalCPI), " \u20AA")), /*#__PURE__*/React.createElement("div", {
    className: "tax-cpi-metric tax-cpi-metric--saving",
    style: {
      padding: 12,
      borderRadius: 10,
      background: "rgba(76,175,80,0.08)",
  ...
  }, "\u05D7\u05D9\u05E1\u05DB\u05D5\u05DF \u05E4\u05D5\u05D8\u05E0\u05E6\u05D9\u05D0\u05DC\u05D9"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 20,
      fontWeight: 800,
      color: "#5ab85a"
    }
  }, "+", fm(cpiSim.totalSavings), " \u20AA"))), /*#__PURE__*/React.createElement("button", {
    onClick: () => setCpiExpanded(!cpiExpanded),
    style: {
      width: "100%",
      padding: "8px",
      borderRadius: 8,
  ...
  }, cpiExpanded ? "▲ הסתר פירוט" : "▼ הצג פירוט לפי נייר ערך"), cpiExpanded && /*#__PURE__*/React.createElement("div", {
  ...
  }, ["נייר ערך", "מטבע", "הצמדה", "רווח לא ממומש", "מס 25%", "מס ריאלי", "חיסכון"].map(h => /*#__PURE__*/React.createElement("th", {
    key: h,
    style: {
      padding: "8px 6px",
      textAlign: "center",
  ...
  }, h)))), /*#__PURE__*/React.createElement("tbody", null, cpiSim.rows.map(row => /*#__PURE__*/React.createElement("tr", {
    key: row.symbol,
    style: {
  ...
  }, row.adjustmentType === 'cpi' ? /*#__PURE__*/React.createElement("span", {
    style: {
      color: "#5ab85a",
      fontWeight: 700,
      padding: "2px 6px",
  ...
  }, "\u05DE\u05D3\u05D3") : row.adjustmentType === 'fx' ? /*#__PURE__*/React.createElement("span", {
    style: {
      color: "hsl(210,70%,65%)",
      fontWeight: 700,
      padding: "2px 6px",
    ...
    style: {
      padding: "7px 6px",
      textAlign: "center",
      color: row.unrealizedGain >= 0 ? "#5ab85a" : "#e07070"
    }
  }, row.unrealizedGain >= 0 ? `+${fm(row.unrealizedGain)}` : fm(row.unrealizedGain), " \u20AA"), /*#__PURE__*/React.createElement("td", {
    style: {
      padding: "7px 6px",
      textAlign: "center",
      color: "#e07070"
    }
  }, fm(row.flatTax), " \u20AA"), /*#__PURE__*/React.createElement("td", {
    style: {
      padding: "7px 6px",
      textAlign: "center",
      color: "hsl(210,70%,65%)"
    }
  }, fm(row.cpiAdjTax), " \u20AA"), /*#__PURE__*/React.createElement("td", {
    style: {
      padding: "7px 6px",
      textAlign: "center",
      fontWeight: row.savings > 0 ? 700 : 400,
      color: row.savings > 0 ? "#5ab85a" : "hsl(215, 12%, 52%)"
    }
  }, row.savings > 0 ? `+${fm(row.savings)} ₪` : "—"))))), /*#__PURE__*/React.createElement("p", {
    style: {
      fontSize: 11,
      color: "#5a5a7a",
      margin: "10px 0 0",
  ...
  }, "* \u05D4\u05E1\u05D9\u05DE\u05D5\u05DC\u05E6\u05D9\u05D4 \u05DE\u05E0\u05D9\u05D7\u05D4 \u05DE\u05DB\u05D9\u05E8\u05D4 \u05E9\u05DC \u05DB\u05DC \u05D4\u05E0\u05D9\u05D9\u05E8\u05D5\u05EA \u05D1\u05DE\u05D7\u05D9\u05E8 \u05D4\u05E0\u05D5\u05DB\u05D7\u05D9. \u05D7\u05D9\u05E9\u05D5\u05D1 \u05D4-CPI \u05DE\u05D1\u05D5\u05E1\u05E1 \u05E2\u05DC \u05DE\u05D3\u05D3 \u05D4\u05DC\u05DE\"\u05E1 120010. \u05E2\u05D3\u05DB\u05DF \u05DE\u05D7\u05D9\u05E8\u05D9\u05DD \u05D1\u05D3\u05E3 \u05D4\u05E4\u05D5\u05E8\u05D8\u05E4\u05D5\u05DC\u05D9\u05D5 \u05DC\u05E4\u05E0\u05D9 \u05D4\u05D7\u05D9\u05E9\u05D5\u05D1.", cpiSim.rows.some(r => !r.lotsFromHistory) && " חלק מהניירות מחושבים לפי costBasis ממוצע (אין היסטוריית קניות מפורטת).")))), /*#__PURE__*/React.createElement("button", {
    className: "tax-view tax-view--plan tax-detail-toggle",
    onClick: () => setDet(!det),
    style: {
      width: "100%",
      padding: "12px",
  ...
  }, det ? "▲ הסתר" : "▼ הצג", " \u05D8\u05D1\u05DC\u05EA \u05E0\u05E7\u05D5\u05D3\u05D5\u05EA \u05D6\u05D9\u05DB\u05D5\u05D9 \u05DC\u05E4\u05D9 \u05D2\u05D9\u05DC (\u05E8\u05E4\u05D5\u05E8\u05DE\u05EA 2024)"), det && /*#__PURE__*/React.createElement("div", {
    className: "tax-panel tax-view tax-view--plan tax-table-wrap",
    style: {
      background: "rgba(255,255,255,0.035)",
      borderRadius: 14,
      border: "1px solid rgba(255,255,255,0.07)",
  ...
  }, ["גיל", "אב בסיס", "רפורמה", "אב סה״כ", "אם בסיס", "רפורמה", "אם סה״כ"].map(h => /*#__PURE__*/React.createElement("th", {
    key: h,
    style: {
      padding: "8px 4px",
      textAlign: "center",
  ...
  }, h)))), /*#__PURE__*/React.createElement("tbody", null, [["לידה", 1.5, 1, 2.5, 1.5, 1, 2.5], ["1-2", 2.5, 2, 4.5, 2.5, 2, 4.5], ["3", 2.5, 1, 3.5, 2.5, 1, 3.5], ["4-5", 2.5, 0, 2.5, 2.5, 0, 2.5], ["6-17", 0, 0, 0, 1, 0, 1], ["18", 0, 0, 0, 0.5, 0, 0.5]].map(row => /*#__PURE__*/React.createElement("tr", {
    key: row[0],
    style: {
      borderBottom: "1px solid rgba(255,255,255,0.05)"
    }
    ...
    style: {
      textAlign: "center",
      color: row[2] ? "#5ab85a" : "#4a4a5a"
    }
  }, row[2] ? `+${row[2]}` : "\u2014"), /*#__PURE__*/React.createElement("td", {
    style: {
      textAlign: "center",
      color: "#7ab8e0",
      fontWeight: 700
    }
  }, row[3]), /*#__PURE__*/React.createElement("td", {
    style: {
      textAlign: "center",
      color: "#d898c8"
    }
  }, row[4]), /*#__PURE__*/React.createElement("td", {
    style: {
      textAlign: "center",
      color: row[5] ? "#5ab85a" : "#4a4a5a"
    }
  }, row[5] ? `+${row[5]}` : "\u2014"), /*#__PURE__*/React.createElement("td", {
    style: {
      textAlign: "center",
      color: "#d898c8",
      fontWeight: 700
    }
  }, row[6]))))), /*#__PURE__*/React.createElement("p", {
    style: {
      fontSize: 11,
      color: "#6a6a8a",
      marginTop: 10,
  ...
  }, "* \u05E2\u05D3 \u05D2\u05D9\u05DC 5 \u05E1\u05D4\u05F4\u05DB \u05D6\u05D4\u05D4 \u05DC\u05E9\u05E0\u05D9 \u05D4\u05D4\u05D5\u05E8\u05D9\u05DD (\u05DE\u05E1\u05E2\u05D9\u05E4\u05D9\u05DD \u05E9\u05D5\u05E0\u05D9\u05DD). \u05DE\u05D2\u05D9\u05DC 6 \u05E8\u05E7 \u05D4\u05D0\u05DD \u05DE\u05E7\u05D1\u05DC\u05EA.")), /*#__PURE__*/React.createElement("section", {
    className: "tax-panel tax-view tax-view--plan tax-steps",
    style: {
      background: "rgba(255,255,255,0.035)",
      borderRadius: 14,
      border: "1px solid rgba(255,255,255,0.07)",
  ...
  }, "\uD83D\uDDC2\uFE0F \u05E6\u05E2\u05D3\u05D9\u05DD"), [["1", "הגשת בקשה למענק עבודה", "שני בני הזוג בנפרד, לשנת 2024 (עד 31.12.26) ו-2025 (עד 31.12.27)"], ["2", "בדיקת רווחים צבורים", "בקש טופס 867 מהבנק/ברוקר"], ["3", `מימוש עד ${fm(r.hOpt)} ₪`, "לפני 30.12 (אירופה) / 31.12 (ארה״ב) — קנה מחדש מיד"], ["4", "הגשת דוח שנתי", "עם רו״ח, כלול 867 + כל ההכנסות → החזר מס"], ["5", "עדכון טופס 101", "פרטי ילדים + ישוב מזכה"]].map(([n, t, d]) => /*#__PURE__*/React.createElement("div", {
    className: "tax-step-row",
    key: n,
    style: {
      display: "flex",
      gap: 10,
    ...
    className: "tax-view tax-view--plan tax-disclaimer",
    style: {
      padding: "12px",
      borderRadius: 10,
  ...
  }, /*#__PURE__*/React.createElement("strong", null, "\u26A0\uFE0F"), " \u05E1\u05DB\u05D5\u05DE\u05D9 \u05D4\u05DE\u05E2\u05E0\u05E7 \u05D4\u05DD \u05D4\u05E2\u05E8\u05DB\u05D4. \u05D4\u05DB\u05E0\u05E1\u05EA \u05D1\u05DF/\u05D1\u05EA \u05D4\u05D6\u05D5\u05D2 \u05DE\u05E9\u05E4\u05D9\u05E2\u05D4 \u05E2\u05DC \u05D4\u05DE\u05E2\u05E0\u05E7. \u05E0\u05E7\u05D5\u05D3\u05D5\u05EA \u05D4\u05D6\u05D9\u05DB\u05D5\u05D9 \u05DE\u05D3\u05D5\u05D9\u05E7\u05D5\u05EA \u05DC\u05E4\u05D9 \u05E8\u05E4\u05D5\u05E8\u05DE\u05EA 2024. \u05DE\u05D3\u05E8\u05D2\u05D5\u05EA \u05D4\u05DE\u05E1 \u05D5\u05E9\u05D5\u05D5\u05D9 \u05E0\u05E7\u05D5\u05D3\u05EA \u05D6\u05D9\u05DB\u05D5\u05D9 \u05E2\u05E9\u05D5\u05D9\u05D9\u05DD \u05DC\u05D4\u05E9\u05EA\u05E0\u05D5\u05EA \u05DE\u05E9\u05E0\u05D4 \u05DC\u05E9\u05E0\u05D4 \u2014 \u05E2\u05D3\u05DB\u05DF \u05D1\u05D4\u05EA\u05D0\u05DD. \u05D0\u05D9\u05DF \u05DC\u05E8\u05D0\u05D5\u05EA \u05D1\u05D6\u05D4 \u05D9\u05D9\u05E2\u05D5\u05E5 \u05DE\u05E1 \u2014 \u05D4\u05EA\u05D9\u05D9\u05E2\u05E6\u05D5 \u05E2\u05DD \u05E8\u05D5\u05F4\u05D7.")));
}
