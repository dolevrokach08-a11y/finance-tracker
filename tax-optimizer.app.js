/* GENERATED from tax-optimizer.src.jsx — source sha256:900aecd7d261f539
...
function App() {
  ...
  const confirmForm106Upload = useCallback(async () => {
    ...
    if (savedCount > 0) {
      ...
      const allSlips = (window.__payslipData.payslips || []).filter(p => p.earner === earner && p.month && p.month.startsWith(year + '-') && p.gross);
  ...
  const confirmPayslipUpload = useCallback(async () => {
    ...
    if (ok) {
      ...
      if (payslip.gross) {
        ...
        if (payslip.earner === 'father') setFM(rounded);else if (payslip.earner === 'mother') setMM(rounded);
      }
  ...
  }, payslips.sort((a, b) => (b.month || '').localeCompare(a.month || '')).map(p => /*#__PURE__*/React.createElement("div", {
  ...
  }, p.month, " ", p.earner === 'father' ? '👨' : p.earner === 'mother' ? '👩' : ''), /*#__PURE__*/React.createElement("span", null, /*#__PURE__*/React.createElement("span", {
    style: {
      color: "hsl(215, 12%, 52%)"
    }
  }, "\u05D1\u05E8\u05D5\u05D8\u05D5: "), /*#__PURE__*/React.createElement("strong", {
